...
-- 获取应用私有目录存储空间大小剩余大小（android appVerison 1.33 之后才支持 ）
function TFDeviceInfo:getAppPrivateFreeSpace()
    local ok = false
    local value = nil
    if CC_TARGET_PLATFORM == CC_PLATFORM_IOS then
        ok,value = TFLuaOcJava.callStaticMethod(TFDeviceInfo.CLASS_NAME,"getDeviceFreeSpace")
    else
        ok,value = TFLuaOcJava.callStaticMethod("org/cocos2dx/lib/Cocos2dxHelper","getAppPrivateFreeSpace",nil,"()I")
    end
    return TFDeviceInfo.checkResult(ok,value)
end
...
--是否是谷歌模拟器PC版本
function TFDeviceInfo:isGooglePC()
    if me.platform ~= "android" then return false end
    local ok,ret = TFLuaOcJava.callStaticMethod(TFDeviceInfo.CLASS_NAME, "isGooglePC", nil, "()Z")
    return TFDeviceInfo.checkResult(ok, ret)
end
