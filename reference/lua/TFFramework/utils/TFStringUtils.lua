...
function string.startswith(str, prefix)
    -- 检查前缀是否为空，或者字符串长度小于前缀长度
    if prefix == "" then return true end
    if #str < #prefix then return false end
    -- 截取字符串的前 #prefix 个字符并与前缀比较
    return string.sub(str, 1, #prefix) == prefix
end

function string.isNullOrEmptyOrZero(str)
	return str == nil or str == string.Empty or str == "0"
end
