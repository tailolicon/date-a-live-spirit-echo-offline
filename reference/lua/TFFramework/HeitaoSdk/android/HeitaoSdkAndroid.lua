...
HeitaoSdk.nativeeventcallback = nil
...
function HeitaoSdk.callback(result, msg)
    ...
    elseif result == HeitaoSdk.NATIVE_EVENT then
        if HeitaoSdk.nativeeventcallback then
             HeitaoSdk.nativeeventcallback(msg)
        end
    end
end
...
--用户中心
function HeitaoSdk.userCenter()
    local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "userCenter", nil, "()Z")
    if ok then 
        if ret == 1 then
            return true
        end 
...
--获取应用事件 用于跳转活动
function HeitaoSdk.getevent()
    local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "getevent", nil, "()Ljava/lang/String;")
    return HeitaoSdk.checkResult(ok,ret)
end

--清除活动事件
function HeitaoSdk.cleanevent()
    local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "cleanevent", nil, "()V")
end


-- function HeitaoSdk.unlockAchievement(achievementId)
--     if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
--        local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "unlockAchievement", {name}, "(Ljava/lang/String;Ljava/lang/String;)V");
--        return HeitaoSdk.checkResult(ok,ret)
--     end
-- end

-- function HeitaoSdk.setStepsImmediate(achievementId ,steps)
--     if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
--         local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "setStepsImmediate", {achievementId ,steps}, "()Ljava/lang/String;")
--         return HeitaoSdk.checkResult(ok,ret)
--     end
-- end
-- 同步成就 
--[[
local achievements  = {id type steps}  

--]]

function HeitaoSdk.syncAchievement(jsonData)
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
       local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "syncAchievement", {jsonData}, "(Ljava/lang/String;)V");
       return HeitaoSdk.checkResult(ok,ret)
    end
end

--限时成就界面
function HeitaoSdk.showAchievements()
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
       local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "showAchievements", nil, "()V");
       return HeitaoSdk.checkResult(ok,ret)
    end
end

--PlayGame登录调用
function HeitaoSdk.playGameSignIn()
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
       local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "playGameSignIn", nil, "()V");
       return HeitaoSdk.checkResult(ok,ret)
    end
end

--载入成就
function HeitaoSdk.loadAchievements()
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
       local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "loadAchievements", nil, "()V");
       return HeitaoSdk.checkResult(ok,ret)
    end
end

function HeitaoSdk.isPlayGameSignIn()
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
       local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "isPlayGameSignIn", nil, "()Z");
       return HeitaoSdk.checkResult(ok,ret)
    end
end
