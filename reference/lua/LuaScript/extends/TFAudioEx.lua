...
local function __checkAudioFile(filename)
	-- __print("---------------------------------------------------------------------------checkFilePath:" ..tostring(filename))
	if string.isNullOrEmpty(filename)  then 
		return false
	end
	if not string.match(filename ,"%.mp3$") then 
		return false
	end
	...
	if not TFFileUtil:existFile(filename) then
		...
		-- __print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>checkFilePath ok:" ..tostring(filename))
	return true
end
...
function M.playMusic(filename, isLoop)
	...
	if not __checkAudioFile(filename) then 
		return -1
	end
...
function M.setEffectsVolumeById( handlerId, volume )
	...
	if useNewAudioEngine then 
		if handlerId and handlerId ~= -1 then 
...
function M.setFinishCallback(handle, callback)
	...
	if callback then 
		local timer
		timer = TFDirector:addTimer(3000, 1, function()
            callback()
