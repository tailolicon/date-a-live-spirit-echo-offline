...
function Utils:tranFileSize(filesize)
    local filesizestr = "0kb"
    if filesize < 1048576 then
        return string.format("%.2fkb",filesize/1024)
    else
        return string.format("%.2fMb",filesize/1048576)
    end
end
...
function Utils:showTips(idOrText, ...)
    ...
    if id then
        text = TextDataMgr:getText(id, unpack(args))
    else
        if #args > 0 then
            text = string.format(text, unpack(args))
        end
    end
...
--战斗准备界面
function Utils:createHeroModel_(node, skinId ,_setName)
    local skinInfo  = TabDataMgr:getData("HeroSkin", skinId)
    local modelInfo = TabDataMgr:getData("HeroModle",skinInfo.paint)
    local modelSet  = modelInfo[_setName] or {scale=0.45,x=50,y=-280}
    local scale     = modelSet.scale
    local offset    = ccp(modelSet.x,modelSet.y)
    if node.__model and node.__heroId == heroId and node.__skinId == skinId then
        node.__model:setScale(scale)
        node.__model:setPosition(offset)
        return  node.__model
    else
        if node.__model then 
            node.__model:removeFromParent()
        end
        node.__model = nil
        node.__heroId = nil
        node.__skinId = nil
    ...
    node.__model = model
    node.__heroId = heroId
    node.__skinId = skinId
    node:addChild(model)
    return model
end
...
function Utils:nextADImage()
    if not self._adImages then 
        self._adImages = {}
        local datas  = TabDataMgr:getData("AdWeight")
        for i, data in pairs(datas) do
            if data.showType == 1 then
                table.insert(self._adImages,data.res)
            end
        end
    end
    if self._adIdx then 
        self._adIdx = self._adIdx + 1
        if self._adIdx > #self._adImages then 
            self._adIdx = 1
        end
    else
        self._adIdx = math.random(1,#self._adImages)
        dump(self._adImages)
    end
    return self._adImages[self._adIdx]
end
...
--日志上报
function Utils:sendHttpLog(flagStr, isClick ,cdnIndex,errorCode,info)
    ---暂时屏打点功能
    -- if true then
    --     return
    -- end
    ...
    data.gaid = ""
    data.networkCarrier = (TFDeviceInfo:getCarrierOperator() or "") --网络运营商 
    ...
    if cdnIndex then 
        ...
        data.property.hostname = cdnIndex
    end 
    if errorCode then 
        data.property.errorCode = errorCode
    end 
    if info then
...
function Utils:showAnnouncement()
...
function Utils:setFontName(label)
    local code = TFLanguageMgr:getUsingLanguage()
    if code == cc.SIMPLIFIED_CHINESE or code == cc.TRADITIONAL_CHINESE then
        if label then
            label:setFontName("font/fangzheng_zhunyuan.ttf")
        end
    end
end

--是否是临时屏蔽渠道
function Utils:isTemporaryChannel()
    if HeitaoSdk then 
        return HeitaoSdk.getplatformId() == "4" 
    end
end


function Utils:addDiscountLabel( node ,targetPos)
    -- if not node._imgDiscount then 
    --     node._imgDiscount  = TFImage:create("ui/activity/firstRecharge/biaoqian.png")
    --     node:addChild(node._imgDiscount,1)    
    --     node._imgDiscount:setPosition(targetPos)
    -- end
end
...
function Utils:isCH()
    ...
    return usingLanguage == cc.SIMPLIFIED_CHINESE or usingLanguage == cc.TRADITIONAL_CHINESE 
end

--同步成就
function Utils:syncAchievement(achievementDatas)
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
        if tonumber(TFDeviceInfo:getCurAppVersion()) >= 1.32 then 
            if HeitaoSdk then 
                local jsonData = json.encode(achievementDatas)
                HeitaoSdk.syncAchievement(jsonData)
            end
        end
    end
end
...
--调用登录PlaygameService
function Utils:playGameSignIn()
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
        if tonumber(TFDeviceInfo:getCurAppVersion()) >= 1.32 then 
            if HeitaoSdk then 
                HeitaoSdk.playGameSignIn()
            end
        end
    end
end
...
--周年庆UI皮肤
function Utils:getZNQUISkin()
     --0（默认原始皮肤） 1 新皮肤
    return 1
end

Utils:sendHttpLog("gamestart")

TFDirector:addMEGlobalListener("applicationDidEnterBackground", function ()
    Utils:sendHttpLog("applicationDidEnterBackground")
end)

TFDirector:addMEGlobalListener("applicationWillEnterForeground", function() 
    Utils:sendHttpLog("applicationWillEnterForeground")
end )
