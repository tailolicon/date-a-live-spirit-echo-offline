...
function BattleDataMgr:transData(roleType,data,job,limitCid,manaInfo, assignCamp)
    job = job or 0
    ...
    elseif roleType == eRoleType.Team then
        ...
        newData.lvl = data.lvl
        newData.exp = data.exp
        newData.angelStrengthen =  data.angelStrengthen
        newData.strName  = TextDataMgr:getText(newData.name)
        newData.attr  = {}
        --属性格式转换为统一格式
        for i, attrInfo in ipairs(data.attr) do
    ...
    elseif roleType == eRoleType.Hero_Arena then
        ...
        --dump(data.attr)
        -- print("---------- data.attr")
        for i, attrInfo in pairs(data.attr) do
            if type(attrInfo) == 'number' then
                newData.attr[i] = attrInfo
            else 
                newData.attr[attrInfo.type] = attrInfo.val
            end
        end
        ...
        newData.camp = assignCamp   -- 1 己方  71 敌方
        if assignCamp == 1 then 
            newData.dir = eDir.RIGHT
            newData.roleType    = eRoleType.Hero
        else
            newData.dir = eDir.LEFT
            newData.roleType    = eRoleType.Hero_Arena
        end
    end
    ...
    for pos = 1 , _max_num do
        if self.formation_ then
            ...
            if formation then
                ...
                data.portraitFrameId = AvatarDataMgr:getCurUsingFrameCid()
                --初始玩家头像列表显示顺序
                if job == 1 then 
...
--竞技场战斗附加霸体能量恢复
function BattleDataMgr:heroData_arena_addtion(data)
    local segment    = ArenaDataMgr:getSegment()
    local segmentCfg = TabDataMgr:getData("RankReward" ,segment)
    data.stiffness   = segmentCfg.stiffness
    data.recover1    = segmentCfg.recover1
    data.recover2    = segmentCfg.recover2
    data.touchBuff   = segmentCfg.touchBuff
    data.recoverBuff = segmentCfg.recoverBuff
end
--竞技场英雄数据 (job 有特殊意义 用于标识角色的出战顺序 1-4 2-5 3-6)
function BattleDataMgr:heroData_arena(isOwn)
    local rawDataList = {}
    local dataList = {}


    local targetData = ArenaDataMgr:getMatchData()
    for pos = 1 , 3 do
        local heroInfo = targetData.selfHeroes[pos]
        if heroInfo then
            local data     = self:transData(eRoleType.Hero_Arena, heroInfo, job , nil , nil ,1)
            table.insert(rawDataList, clone(data))
            data.limitType = EC_BattleHeroType.OWN
            data.job = pos
            data.pid = MainPlayer:getPlayerId()
            data.pname = MainPlayer:getPlayerName()
            data.headShowIndex = pos
            --用户头像框ID
            data.portraitFrameId = AvatarDataMgr:getCurUsingFrameCid()
            self:heroData_arena_addtion(data)
            --初始玩家头像列表显示顺序
            table.insert(dataList, data)
        else
            Box("not found arena self formation data : " .. tostring(pos))
        end
    end
    ...
    -- local targetData = ArenaDataMgr:getMatchData()
    for pos = 1 , 3 do
        local heroInfo = targetData.heroes[pos]
        if heroInfo then
            local data     = self:transData(eRoleType.Hero_Arena, heroInfo, job , nil , nil ,71)
            data.limitType = EC_BattleHeroType.OWN
            data.job = 3 + pos
            data.pid = targetData.pid
            data.pname = targetData.pName
             --用户头像buffs
            data.portraitFrameId = 0 
            self:heroData_arena_addtion(data)
            --初始玩家头像列表显示顺序
            table.insert(dataList, data)
        else
            Box("not found arena target formation data : " .. tostring(pos))
        end
    end


    -- Box("dataList size :" ..tostring(#dataList))
    return dataList
end
...
function BattleDataMgr:setServerData(sData, battleType)
    ...
    -- Box("setServerData: "..tostring(battleType))
    ...
    elseif battleType == EC_BattleType.ARENA then -- 竞技场战斗
        data.map    = levelCfg.levelScript
        data.heros  =  self:heroData_arena(true) -- self:heroData()
        ...
        -- dump(levelCfg.victoryType) --22
        data.roundData  = {}
        for i=1,3 do
            data.roundData[i] = { 
                selfHero  = data.heros[i].id ,
                enemyHero = data.heros[i+3].id ,
                success   = true
             }
        end
        dump(data.roundData)
        -- Box("RoundData")
        --data.targetHeros =  self:heroData_arena(false)  --组织角色数据
    end
