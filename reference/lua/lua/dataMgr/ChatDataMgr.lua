...
function ChatDataMgr:onRecvChatInfo(event)
    ...
    chatInfo.messageId = self:nextMessageId()
...
function ChatDataMgr:onChangeRoom(event)
    ...
    for i,v in ipairs(chatInfo_) do
        v.messageId = self:nextMessageId()
...
function ChatDataMgr:onRecvLeagueHistoryChat(event)
    print_(event.data)
    print_("onRecvLeagueHistoryChat")
end
...
function ChatDataMgr:onChatRecordWorldInfo(event)
    ...
    for i,v in ipairs(chatInfos) do
        local chatInfo = v
        chatInfo.messageId = self:nextMessageId()
        chatInfo.chatType = EC_ChatType.WORLD
        table.insert(chatInfoList,1,chatInfo)
    end
    -- dump(chatInfos)
    -- Box("onChatRecordWorldInfo")
end
...
-- --翻译测试
-- function ChatDataMgr:translate1(text)
--     local UserCenterHttpClient = TFClientNetHttp:GetInstance()
--     local url  = "https://translation.googleapis.com/language/translate/v2?key=AIzaSyBpZwWQzGiA3OyTd2sPts7JLS5q6hmC1io"
--     local code = "zh"
--     local t = {
--         q   = {},
--         target= code
--     }
--     table.insert(t.q,text)
--     local postData = json.encode(t)
--     print_(postData)
--     local header = "Content-Type: application/json; charset=utf-8"
--     UserCenterHttpClient:addMERecvListener(function(_type,ret,data)
--         dump({_type,ret,data})
--         print_("----收到数据")

--     end)
--     UserCenterHttpClient:setNetLogEnable(true)
--     UserCenterHttpClient:httpRequest(TFHTTP_TYPE_POST,url ,postData,header)
-- end

function ChatDataMgr:nextMessageId()
    self._messageId = self._messageId or 0
...
--返回翻译映射的语音
function ChatDataMgr:getTraslateCode(language)
    if  not self.languageCodeMap then   
        self.languageCodeMap = {}
        self.languageCodeMap[cc.FRENCH]     = "fr"
        self.languageCodeMap[cc.GERMAN]     = "de"
        self.languageCodeMap[cc.SPANISH]    = "es"
        self.languageCodeMap[cc.THAI]       = "th"
        self.languageCodeMap[cc.INDONESIAN] = "id"
        self.languageCodeMap[cc.KOREAN]     = "ko"
        self.languageCodeMap[cc.TRADITIONAL_CHINESE] = "zh-TW"
        self.languageCodeMap[cc.ENGLISH]             = "en"
        self.languageCodeMap[cc.SIMPLIFIED_CHINESE]  = "zh-CN"
        self.languageCodeMap[cc.VIETNAM]             = "vi"
    end
    return self.languageCodeMap[TFLanguageMgr:getUsingLanguage()]  or "en"
end


-- translate()
-- 翻译接口测试
function ChatDataMgr:translate(_chatInfo)
    if _chatInfo.tState  and  _chatInfo.tState  > 0  then
        return 
    end
    -- text = "Hello world!"
    -- dump(_chatInfo)
    -- local chatType = chatInfo.chatType
    local chatInfo = self:getChatInfo(_chatInfo.chatType , _chatInfo.messageId)
    if not chatInfo then
        Box("not found message :"..tostring(_chatInfo.messageId)) 
        return 
    end
    local text   = chatInfo.content
    local target = self:getTraslateCode(TFLanguageMgr:getSdkUsingLanguage())
    local url    = "https://translation.googleapis.com/language/translate/v2?key=AIzaSyBpZwWQzGiA3OyTd2sPts7JLS5q6hmC1io&q="..string.url_encode(text).."&target="..target
    -- if  CC_TARGET_PLATFORM == CC_PLATFORM_WIN32 then --TODO pc 内网代理翻译
    --     url  = "http://192.168.22.236:8080/translate/v2?key=AIzaSyBpZwWQzGiA3OyTd2sPts7JLS5q6hmC1io&q="..string.url_encode(text).."&target="..target
    -- end
    print_(url)
    chatInfo.tState  = 1 --标记为翻译状态
    _chatInfo.tState = 1
    HttpHelper:get(url, function (data)
        print_("translate:" ..tostring(data))
        if self:onTranslateCallBack(data , chatInfo) then
            chatInfo.tState  = 2
            _chatInfo.tState = 2
            --更新本地消息
            EventMgr:dispatchEvent(EV_RECV_TRANSLATE)
        else
            chatInfo.tState  = 0
            _chatInfo.tState = 0
        end
...
function ChatDataMgr:onTranslateCallBack(data ,chatInfo)
    if not string.isNullOrEmpty(data) then 
        local jsonObj        = json.decode(data)
        -- dump(jsonObj)
        --单条翻译
        if jsonObj.data and  jsonObj.data.translations and #jsonObj.data.translations > 0 then
            local text = jsonObj.data.translations[1].translatedText
            if not string.isNullOrEmpty(text) then 
                chatInfo.tContent = text
                return true
            end
        end
    end
...
function ChatDataMgr:getChatInfo(chatType,messageId)
    local channelDatas = self.datas[chatType] or {}
    -- print("chatType: " ..tostring(chatType))
    -- dump(channelDatas)
    -- dump(self.datas)
    for i, v in ipairs(channelDatas) do
        -- print(" xxx  "..tostring(v.messageId) .. "  : " ..tostring(messageId) .."  ret: " ..tostring( v.messageId == messageId ))
...
-- --解析翻译出的文本
-- local function parse(jsonText)
--     local jsonObj        = json.decode(jsonText)
--     local translatedText = jsonObj.data.translations[1].translatedText
-- end
