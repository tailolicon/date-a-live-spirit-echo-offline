...
function RechargeDataMgr:getOrderNO(goodsid, extraInfo)
	...
	if goods.buyType and goods.buyType == 1 then
		...
		local price = goods.rechargeCfg.price
		self.curPayId = goodsid;
		self:exchangeConfirm(goodsid, price ,extraInfo)		
		-- local msg = {
		-- 	goodsid,
		-- 	extraInfo
		-- }
...
function RechargeDataMgr:exchangeConfirm(goodsid, price ,extraInfo)

    function _cashPurchase()
        local msg = {
            goodsid,
            extraInfo
        }
    ...
    ---消费卷数量
    local count = GoodsDataMgr:getItemCount(666000)
    if count < price  then --不够兑换直接购买 
    	_cashPurchase()
    	return
    end
    function _exchange()
        ...
        TFDirector:send(c2s.RECHARGE_REQ_RECHARGE_EXCHANGE, msg)
    end
    local view = Utils:openView("common.ConfirmBoxView")
    view:setTitle(TextDataMgr:getText(14210305)) 
    view:setContent(TextDataMgr:getText(18000305,tostring(price)))
    local Panel_goodsItem = PrefabDataMgr:getPrefab("Panel_goodsItem"):clone()
    PrefabDataMgr:setInfo(Panel_goodsItem,666000, count)
    Panel_goodsItem:Scale(0.7):Pos(0, 0):AddTo( view.Image_tips_icon)
    view:setCallback(_exchange)
    view:setCancelCallback(_cashPurchase)
...
function RechargeDataMgr:recvGetOrderNo(event)
	...
	--end]]
	local productId  = self.curPayId
    if cfg.extendData then
        local jsonData = Json.decode(cfg.extendData)
        if jsonData.android_sku then
        	productId = jsonData.android_sku
		else
			dump(cfg)
			Box("未配置谷歌商店商品ID")
        end
	...
	if HeitaoSdk then
		HeitaoSdk.pay(tostring(productId),
			cfg.name2,
			cfg.des3,
			...
			tostring(ServerDataMgr:getServerID_()), 
			"1",
			"1",
