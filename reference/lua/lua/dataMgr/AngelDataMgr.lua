...
function AngelDataMgr:ctor()
	...
	local _angelStrengthen = TabDataMgr:getData("AngelStrengthen")


	self.angelStrengthenTable = {}
	for k,v in pairs(_angelStrengthen) do
		self.angelStrengthenTable[v.heroId] = self.angelStrengthenTable[v.heroId] or {}
		self.angelStrengthenTable[v.heroId][v.skillType] = self.angelStrengthenTable[v.heroId][v.skillType] or {}
		-- self.angelStrengthenTable[v.heroId][v.skillType][v.lvl] = self.angelStrengthenTable[v.heroId][v.skillType][v.lvl] or 
		self.angelStrengthenTable[v.heroId][v.skillType][v.lvl] = v
		-- self.angelStrengthenTable[v.heroId][v.skillType][v.lvl] = v
	end
...
function AngelDataMgr:init()
	...
	--天使强化
	TFDirector:addProto(s2c.HERO_RES_ANGEL_STRENGTHEN, self, self.recvAngleStrengthen)
end
...
function AngelDataMgr:getSkillIcon(heroId ,skillType)
    local datas = self:getAngleBaseData(heroId,skillType)
    for _k,_v in pairs(datas) do
        local skillTree   = self:getAngleConfig(_v[1]) 
        local skillInfo   = TabDataMgr:getData("Skill",skillTree.skillId[1])
        return skillInfo.icon
    end
    return "icon/skill/10101_skillF.png"
end

--天使对应技能类型的强化等级
function AngelDataMgr:getStregthenLevel(heroId,skillType)
	local heroData = HeroDataMgr:getHero(heroId)
	-- dump(heroData)
	-- dump({heroId,skillType ,heroData.angelStrengthen})
	if heroData.angelStrengthen then 
		for i,v in ipairs(heroData.angelStrengthen) do
...
--天使强化buffer
function AngelDataMgr:getAngelStregthenBuffers(heroData)
	-- dump(heroData)
	local affixeIDs = {}
	-- dump({heroId,skillType ,heroData.angelStrengthen})
	if heroData.angelStrengthen then 
		for i,v in ipairs(heroData.angelStrengthen) do
			local cfg = self:getAngleStrengtheConfig(heroData.id ,v.skillType ,v.lv)
			-- dump((heroData.id ,v.skillType ,v.lv ,cfg})
			if not cfg then 
				Box("not found angelStrengthen heroId: "..tostring( heroData.id) .." skillType: "..tostring(v.skillType).."  Lv: "..tostring(v.lv))
			end
			for i, _affixeID in ipairs(cfg.affixeID) do
				table.insert(affixeIDs , _affixeID)
			end
		end
	end
	return affixeIDs
...
--请求强化天使
function AngelDataMgr:sendAngelStregthen(heroId, skillType,comsumeSelected)
	local costType = comsumeSelected and 2 or 1
    local msg = {tostring(heroId),skillType ,costType}
    -- dump("请求强化天使")
    -- dump(msg)
    TFDirector:send(c2s.HERO_REQ_ANGEL_STRENGTHEN, msg)
end

--天使强化数据返回
function AngelDataMgr:recvAngleStrengthen(event)
	local data = event.data
	-- dump("天使强化数据返回")
    -- dump(data)
	GoodsDataMgr:setHeroAngelStrengthen(data)
	EventMgr:dispatchEvent(EV_HERO_ANGEL_STRENGTHEN,data)
end

function AngelDataMgr:getAngleStrengtheConfig(heroId ,skillType,lvl)
	-- dump({heroId ,skillType,lvl})
	-- dump(self.angelStrengthenTable)
	return self.angelStrengthenTable[heroId][skillType][lvl]
end
...
function AngelDataMgr:recvAngleAwake(event)
