...
function SettingDataMgr:setOptionSkip(value)
    local id = MainPlayer:getPlayerId() or ""
    UserDefalt:setStringForKey("optionSkip"..id, value)
end

function SettingDataMgr:getOptionSkip()
    local id = MainPlayer:getPlayerId() or ""
    local value = UserDefalt:getStringForKey("optionSkip"..id)
    return tonumber(value) or 2
end

function SettingDataMgr:getOptionSkipVal()
    return OPTION_SKIP_VAL[self:getOptionSkip()]
end
...
function SettingDataMgr:bgmFade(to, step, duration, useCurFade)
	...
	local function setVol_(vol)
        if AudioExchangePlay.oldId and AudioExchangePlay.oldId ~= -1 then 
...
function SettingDataMgr:setSummonAnimation(action)
    local id = MainPlayer:getPlayerId() or ""
    UserDefalt:setStringForKey("SummonAnimation"..id, action)
end

function SettingDataMgr:getSummonAnimation()
    local id = MainPlayer:getPlayerId() or ""
    local action = UserDefalt:getStringForKey("SummonAnimation"..id)
    if action == "" then
        return 1
    else
