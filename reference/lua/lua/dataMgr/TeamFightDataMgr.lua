...
function TeamFightDataMgr:checkIsHeroRepeatWithOther(cid)
    ...
    for k,v in pairs(self.tTeamMemberInfo) do
        if v.pid ~= MainPlayer:getPlayerId() and v.status == 2 
            and v.heroCid == cid then
            --and HeroDataMgr:getHeroRoleId(v.heroCid) == HeroDataMgr:getHeroRoleId(cid) then
            return true
        end
    end
    return false
...
function TeamFightDataMgr:inviteFriend(friendid)
    ...
    local content = {affixID = battlecfg.affixID,teamid = self.strTeamId,battlename = battlecfg.levelName,
