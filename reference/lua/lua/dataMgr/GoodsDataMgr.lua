...
function GoodsDataMgr:getItemCount(cid, isFormat)
    ...
    elseif cfg.superType == EC_ResourceType.PET then
        if PetDataMgr:getPetDataByCid(cid) then
            count = 1
        end
...
function GoodsDataMgr:putInBug(id, cid, item)
    ...
    if not itemCfg.bagType or itemCfg.bagType == 0 then return end
...
function GoodsDataMgr:onRecvItemLsit(event)
    ...
    if data.pets then
        ...
        EventMgr:dispatchEvent(EV_BAG_PET_UPDATE)
    end
...
--获取宠物信息
function GoodsDataMgr:getPet(id)
    if id then
        return self.pets_[id]
    else
        return self.pets_
    end
end
...
--宠物处理
function GoodsDataMgr:__petHandle(sItem)
    local function attrAssgin(target, src)
        for k, v in pairs(src) do
            target[k] = v
        end
    ...
    if ct == EC_SChangeType.ADD or ct == EC_SChangeType.DEFAULT then
        local newItem = {}
        attrAssgin(newItem, sItem)
        self.pets_[sItem.id] = newItem
    elseif ct == EC_SChangeType.UPDATE then
        local newItem = {}
        attrAssgin(newItem, sItem)
        self.pets_[sItem.id] = newItem
    elseif ct == EC_SChangeType.DELETE then
        self.pets_[sItem.id] = nil
    end
    PetDataMgr:syncServer(sItem)
end
...
function GoodsDataMgr:setHeroAngelStrengthen(data)
    local hero = self.heros_[data.heroId]
    hero.angelStrengthen = hero.angelStrengthen or {}
    local exist = false
    for k,v in ipairs(hero.angelStrengthen) do
        if v.skillType == data.skillType then
            v.lv = data.lv
            exist = true;
            break
        end
    ...
    if not exist then 
        --组装数据
        local strengthen  = {}
        strengthen.skillType = data.skillType
        strengthen.lv        = data.lv
        table.insert(hero.angelStrengthen,strengthen)
    end
    --同步数据
    HeroDataMgr:syncServer(clone(hero),EC_SChangeType.DEFAULT)


    dump("天使强化更新")
    dump(data)
    dump(hero.angelStrengthen)
end
...
function GoodsDataMgr:resetPet()
    for k, item in pairs(self.pets_) do
        item.ct = EC_SChangeType.DEFAULT
        PetDataMgr:syncServer(clone(item))
    end
end
