...
function TaskDataMgr:init()
    ...
    for k,v in pairs(self.taskMap_) do
        if not string.isNullOrEmpty(v.googleTask) then
            self.achievements_[v.googleTask] = self.achievements_[v.googleTask] or {}
            table.insert(self.achievements_[v.googleTask],v.id)
        end
    end
end
...
function TaskDataMgr:onRecvTask(event)
    ...
    self:checkAchievements(data.taks)
end

--创建
local function createAchievementData( id , _type ,steps ,name)
    local data ={id = id ,type = _type ,steps = steps }
    if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32 then
       data.name = TextDataMgr:getText(name)     
    end
    return data
end

--检查同步成就
function TaskDataMgr:checkAchievements(taks)
    local achievementTables = {}
    for i, v in ipairs(taks) do
        if v.status == 2 then --任务完成
            local taskCfg = self:getTaskCfg(v.cid)
            local achievementId = taskCfg.googleTask
            if not string.isNullOrEmpty(achievementId) then
                if not achievementTables[achievementId] then 
                    local taskCids  = self.achievements_[achievementId] or {}
                    local taskCount = #taskCids
                    if taskCount > 1 then --增长成就
                        local complete = 0
                        for i, taskCid in ipairs(taskCids) do
                            local taskInfo = self.taskInfo_[taskCid]
                            if taskInfo and taskInfo.status == 2 then --完成的任务
                                complete = complete + 1
                            end
                        end
                        local steps = math.floor(complete*100/taskCount)
                        achievementTables[taskCfg.googleTask] = createAchievementData(taskCfg.googleTask,1,steps ,taskCfg.name)
                        --计算百分比
                    elseif taskCount == 1 then --解锁型
                        achievementTables[taskCfg.googleTask] = createAchievementData(taskCfg.googleTask,0,0 ,taskCfg.name)
                    end
                end
            end
        end
    ...
    for k,v in pairs(achievementTables) do
        table.insert(achievementDatas,v)
    end
    
    if #achievementDatas > 0 then
        if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32 then
            local tips = "成就-"
            for i,v in ipairs(achievementDatas) do
                if v.type == 0 then 
                    tips = " [" ..v.id.." "..tostring(v.name) .. " 解锁 ]"
                else
                    tips = " [" ..v.id.." " ..tostring(v.name) .. " step " ..v.steps .."  ]"
                end
            end 
            Utils:showTips(tips)
            dump(achievementDatas)
        end
        -- print("解锁新成就")

        Utils:syncAchievement(achievementDatas)
    end
end
