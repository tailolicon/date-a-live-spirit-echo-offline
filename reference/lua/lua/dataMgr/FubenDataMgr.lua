...
function FubenDataMgr:init()
    ...
    --扫荡数据返回
    TFDirector:addProto(s2c.DUNGEON_RESP_ANNIHILATE, self, self.onRevAnnihilate)
...
function FubenDataMgr:getActivityChapterIsOpen(chapterCid)
    ...
    elseif chapterCid == EC_ActivityFubenType.ARENA then --TODO 竞技场 活动副本
        isOpen = true
...
--是否三星通关关卡
function FubenDataMgr:is3StarPassLevel(levelCid)
    local levelInfo = self.levelInfo_[levelCid]
    if levelInfo then 
        return levelInfo.win and #(levelInfo.goals or {}) >2
    end
end
...
function FubenDataMgr:getAssistHeros()
...
function FubenDataMgr:onRecvFightStart(event)
    ...
    elseif levelCfg.dungeonType == EC_FBLevelType.ENDLESS_PLUSS then
        battleController.enterBattle(data, EC_BattleType.COMMON)
    elseif levelCfg.dungeonType == EC_FBLevelType.ARENA then  --竞技场
        battleController.enterBattle(data, EC_BattleType.ARENA)
    elseif levelCfg.dungeonType == EC_FBLevelType.ELEMENT_TRIAL then --元素试炼
        battleController.enterBattle(data, EC_BattleType.COMMON)
        -- Box("元素试炼战斗")
    end
    self:cachePlayerInfo()
...
-----------主线扫荡功能
function FubenDataMgr:SEND_DUNGEON_REQ_ANNIHILATE( levelCid, quickCount, isDuelMod)
    local content = {levelCid , quickCount, isDuelMod}
    dump(content)
    TFDirector:send(c2s.DUNGEON_REQ_ANNIHILATE,content)
end

function FubenDataMgr:onRevAnnihilate(event)
    local data = event.data
    dump(data)
    local levelInfo = data.levelInfo
    self.levelInfo_[levelInfo.cid] = levelInfo
    local rewards = {} 
    ...
    Utils:openView("bag.GetItemView", rewards)
    EventMgr:dispatchEvent(EV_FUBEN_LEVELINFOUPDATE)
end
