...
function SummonDataMgr:init()
    ...
    --心愿卡设置响应
    TFDirector:addProto(s2c.SUMMON_RES_SET_WISH_ID, self, self.onRecvSetWish)
    ...
    for k, v in pairs(summon) do
        ...
        if info then
            ...
            if info.interfaceType == EC_SummonShowPlaceType.Summon_View then
                ...
                elseif summonType == EC_SummonType.CONTRACT_ROLE or summonType == EC_SummonType.CONTRACT_EQUIP then   
                    table.insert(self.contractSummon, v)
...
--检查契约卡池是否开启
function SummonDataMgr:checkContractSummonOpen(summonId)
    local cfg   = self:getSummonCfg(summonId)
    local goods = GoodsDataMgr:getItem(cfg.ext.lockItem)
    if goods then 
        local _,goodsData = next(goods)
        if goodsData.outTime then
...
function SummonDataMgr:getSummon()
    ...
    --契约卡池
    for i,v in ipairs(self.contractSummon) do
        --TODO 判断卡池对应的道具 是否存在(没有过期)
        if self:checkContractSummonOpen(v[1].id) then 
...
function SummonDataMgr:getSummonByType(summonType)
    local ret = {}
    local summon = clone(self.allSummon_)
    for k,group in pairs(summon) do
        for i,v in ipairs(group) do
            local info = self.summonMap_[v.id]
            if info and info.summonType == summonType then
                table.insert(ret, v)
            end
        end
    end
        ...
        for j,v in ipairs(data) do
            ...
            if cfg then
                if cfg.summonType == EC_SummonType.APPOINT_EQUIPMENT or
                    ...
                    cfg.summonType == EC_SummonType.PET or
...
function SummonDataMgr:onRecvSummon(event)
    ...
    if data.sixGuaranteesCount then
        local summonType = self:getSummonCfg(data.id).summonType
        self.guaranteesDic[summonType] = data.sixGuaranteesCount
    end

    if data.wishId then 
        local poolType = self:getSummonCfg(data.id).poolType
        self.wishDic[poolType] = data.wishId
        -- dump("data.wishId:" ..data.wishId)
        -- dump(self.wishDic)
        -- Box("1") 
    end
...
-- 保底次数
function SummonDataMgr:getGuaranteesCount(type)
   return self.guaranteesDic[type] or 0
end

function SummonDataMgr:getWishSummonPool(summonId)
    local poolType = self:getSummonCfg(summonId).poolType
    -- dump(poolType)
    -- dump(self.wishDic)
    return self.wishDic[poolType] or 0
end

function SummonDataMgr:getPetSummonRemainCount()
    local count      = self:getGuaranteesCount(EC_SummonType.PET) 
    local totalCount = Utils:getKVP(90210, "immortalGetTimes")
    return math.max(0, totalCount - count)
end
    ...
    if data.summonInfo then
        for k,info in ipairs(data.summonInfo) do
            ...
            if summonCfg then
                ...
                local summonType = summonCfg.summonType
                local poolType   = summonCfg.poolType
                if info.preciousCount then
                    self.preciousDic[summonType] = info.preciousCount
                end
                if info.sixGuaranteesCount then
                    self.guaranteesDic[summonType] = info.sixGuaranteesCount
                end
                if info.wishId then 
                    self.wishDic[poolType] = info.wishId
                end 
            end
        end
    end
...
--
function SummonDataMgr:getContractSummon()
    return  self.contractSummon
end
...
function SummonDataMgr:getPetWishCard(summonId)
    local configs = {} 
    local summonCfg = self:getSummonCfg(summonId)
    local summonPool= self.summonPoolMap_
    for p, v in pairs(summonPool) do
        if summonCfg.poolType == v.poolType and v.quality >= 5 then
            if v.itemMap then 
                for itemId , num in pairs(v.itemMap) do
                    local petCfg  = PetDataMgr:getPetCfg(itemId)  
                    if petCfg then 
                        table.insert(configs ,{pool = v.id ,item = petCfg.id })
                    end
                end  
            end
        end
...
--设置心愿卡
function SummonDataMgr:send_SUMMON_REQ_SET_WISH_I(summonId, summonPoolId)
    local poolType  = self:getSummonCfg(summonId).poolType
    summonPoolId = summonPoolId or 0
    -- dump({poolType, summonPoolId})
    TFDirector:send(c2s.SUMMON_REQ_SET_WISH_ID, {poolType, summonPoolId})
end

--心愿卡设置响应
function SummonDataMgr:onRecvSetWish(event)
    local data = event.data
    if not data then
        return
    end
    -- dump(data)
    self.wishDic[data.poolType]  = data.summonPoolId
    -- dump(self.wishDic)
    EventMgr:dispatchEvent(EV_SUMMON_WISH_CHANGE)
end
