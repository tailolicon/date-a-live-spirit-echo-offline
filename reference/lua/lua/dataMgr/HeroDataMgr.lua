...
function HeroDataMgr:ctor()
	...
	--竞技场阵容
	self.arenaformation = {}
	self.arenaformation.heros = {}
	self.arenaformation.stance = {}
	self:init()
end
...
function HeroDataMgr:fixFormation(formation)
	formation.heros = {}
...
function HeroDataMgr:fixFormation_arena(formation)
	formation.heros = {}
	if formation.stance then
		for _id,hero in pairs(self.heroTable) do
			for _pos,sid in pairs(formation.stance) do
...
function HeroDataMgr:revcFormationList(event,isFriend)
	self.formations = event.data.formations or {}
	local formation = {}
	for k,v in pairs(self.formations) do
		if v.type == 1 then --主线阵容
			self.formation = self:fixFormation(v)
		elseif v.type == 3 then --竞技场阵容
			if not isFriend then
				self.arenaformation = self:fixFormation_arena(v) 
			end
		elseif v.type == 2 then --组队阵容 无需处理
		end
	end
	dump(self.formation)
	dump(self.arenaformation)
	self:resetShowList();
end
...
function HeroDataMgr:reveFormationChange(event)
	...
	-- dump(data)
	if data.type == 1 then 
		self.formation = self:fixFormation(data)
		self.myFormation = self.formation
	elseif data.type == 3 then 
		self.arenaformation = self:fixFormation_arena(data) 
	end
...
function HeroDataMgr:getIsFormationOn_arena(_pos)
	if self.arenaformation.stance then
		return self.arenaformation.stance[_pos] ~= nil;
	end
	return false
end

function HeroDataMgr:getHeroIdByFormationPos_arena(_pos)
	--dump(self.arenaformation)
	return self.arenaformation.heros[_pos];
end

function HeroDataMgr:getFormationHeroCnt_arena()
	return table.count(self.arenaformation.stance or {});
end

function HeroDataMgr:getFormationIsFull_arena()
	return table.count(self.arenaformation.stance or {}) >= 3;
end

function HeroDataMgr:checkOnFormationByRole_arena(heroid)
	local roleid = self:getHeroRoleId(heroid);

	for i=1,3 do
		if self:getIsFormationOn_arena(i) then
			local id = self:getHeroIdByFormationPos_arena(i);
			if self:getHeroRoleId(id) == roleid then
				return true;
			end
		end
...
function HeroDataMgr:syncStance_arena()
	self.arenaformation.stance = {}
	for k,v in pairs(self.arenaformation.heros) do
		self.arenaformation.stance[k] = v
	end
end



function HeroDataMgr:setFormationHero_arena(pos,id,oldId,force)
	local hero = self:getHero(id)
	if self.arenaformation.heros[pos] == id then
		table.remove(self.arenaformation.heros,pos);
		hero.job = 4;
		...
		if force then 
			self:syncStance_arena()
		end
		return;
	end

	if not self.arenaformation.heros[pos] and not oldId then
		table.insert(self.arenaformation.heros,id);
		for k,v in pairs(self.arenaformation.heros) do
			self:getHero(v).job = k;
		end
	...
	if oldId then
		...
		oldHreo.job = 4;
		hero.job = pos;
		self.arenaformation.heros[pos] = id
...
function HeroDataMgr:getQualityPic(id,_quality)
    ...
    [8] = "ui/common/hero/quality_fx.png",
	}
...
function HeroDataMgr:getToMaxLevelNeedExp(heroId)
	local curLv = self:getLv(heroId)
	local maxLv = self:getCurLevelMax(heroId)
	local curExp = self:getExp(heroId)
	local result = self:getTwoLevelExpDelta(curLv, maxLv) - curExp
	return result
end

function HeroDataMgr:getTwoLevelExpDelta(startLv, endLv)
	local result = 0
	for i = startLv, endLv - 1 do
		result = result + TabDataMgr:getData("LevelUp", i).heroExp
	end
	return result
end

function HeroDataMgr:isCanLevelUp(heroId)
	local curLv = self:getLv(heroId)
	local maxLv = self:getCurLevelMax(heroId)
	return curLv < maxLv
end
...
--天使强化功能是否开启
function HeroDataMgr:isOpenAngelStrengthen(id)
	return self.heroTable[id].openAngelStrengthen == 1
end
...
function HeroDataMgr:heroOnBattle(sourceHeroId,targetHeroId ,formationType)
	...
	formationType = formationType or 1
	local msg = {
			formationType,
			sourceSid,
			targetSid,
...
function HeroDataMgr:changeDataByLevelCfg(levelCfg_,formationData)
	if formationData then
		...
		friend.formationInfo = { formations = {levelFormation}};
...
function HeroDataMgr:getEvolutionDataByPartition(heroId,partition)
	local tabData = {}
	for i, v in pairs(self.evolutionTable) do
		if partition then
...
function HeroDataMgr:getEnergyToMaxLevelNeedExp()
	if (not self:getEnergyIsCanLevelUp()) then
		return
	end
	local result = self:getEnergyTwoLevelExpDelta(self.spiritInfo_.level, self:getEnergyCanLevelUpMaxLevel()) - self.spiritInfo_.exp
	return result
end

function HeroDataMgr:getEnergyIsCanLevelUp()
	local maxLevel = math.min(self:getHeroEnergyMaxLevel(), self:getEnergyBreakMaxLevel())
	return self.spiritInfo_.level < maxLevel
end

function HeroDataMgr:getEnergyCanLevelUpMaxLevel()
	return math.min(self:getHeroEnergyMaxLevel(), self:getEnergyBreakMaxLevel())
end

function HeroDataMgr:getEnergyTwoLevelExpDelta(startLevel, endLevel)
	local result = 0
	for i = startLevel + 1, endLevel do
		result = result + self:getHeroEnergyLevelCfg(nil, i).EnergyExp
	end
	return result
end
...
function HeroDataMgr:isCanOneKeyBreak()
	local config = TabDataMgr:getData("ContractBreak")[1000 + self.spiritInfo_.grade + 1]
	if not config then
		return false
	end
	return GoodsDataMgr:isGoodsEnough(config.BreakCost)
end
...
function HeroDataMgr:Send_SelectPreTeam(teamId,formationType)
	formationType = formationType or 1
	--TODO 协议更新后接入
	TFDirector:send(c2s.PLAYER_REQ_FORMATION_BACKUP_USE,{teamId,formationType})
end
...
--获取精灵宠物信息
function HeroDataMgr:getPet(heroId)
	local hero = self:getHero(heroId)
	return hero.pet
	-- or {
	--     id     = "400001",
