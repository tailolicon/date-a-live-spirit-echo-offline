...
--添加0.1 折标签
function RechargeMain:addDiscountLabel( node )
    
    local parentNodes = {"cell_token" ,"cell_recharge"}
    for i,v in ipairs(parentNodes) do
        local nodeToken  = TFDirector:getChildByPath(node, v)
        local Button_buy = TFDirector:getChildByPath(nodeToken, "Button_buy")
        Utils:addDiscountLabel( Button_buy ,ccp(-80,40))
    end
end
...
function RechargeMain:initUI(ui)
    ...
    self:addDiscountLabel(self.panel_cell)
    ...
    self.btn_TokenMoney:setPosition(self.btn_monthCard:getPosition());
...
function RechargeMain:updateRechargeUI()
    ...
    self.gridView:AsyncUpdateItem(rechargeList,function ( item, data )
        if data then
            ...
            cell_total:hide()
            cell_item:hide()
            self:updateRechargeItemDiamonds(cell_recharge, data)
        end  
    end)
...
local function _refreshCurrency(node  ,itemData)
        ...
        --dump(data)
        node:show()
        local comid = itemData.id
        local Image_zuan = TFDirector:getChildByPath(node, "Image_zuan")
        ...
        Label_num:setText(itemData.num)
        local Label_x = TFDirector:getChildByPath(node,"Label_x")
...
function RechargeMain:updateRechargeItemToken(item, data, buyType)
    local Label_price = TFDirector:getChildByPath(item,"Label_price")
...
--data.tag = false
    ...
    Label_desc_month:setVisible(false)

    local Label_leftTime = TFDirector:getChildByPath(item,"Label_leftTime")
    Label_leftTime:setVisible(false)
    ...
    Image_double:setVisible(isOpenDoubleCard and false)
   ...
   --Label_title:setVisible(data.tag)
    ...
    --dump({tostring(data.tag),data.tagDes,data.tagDes2,data.buyCount ,RechargeDataMgr:getBuyCount(data.rechargeCfg.id)})
    local buyCount = RechargeDataMgr:getBuyCount(data.rechargeCfg.id)
    if data.tag then
    ...
    local _panel_price_names ={"panel_price","panel_price_other"}
    for i,v in ipairs(_panel_price_names) do
        local panel_price = TFDirector:getChildByPath(item, v)
        if buyCount > 0 then 
            _refreshCurrency(panel_price ,data.item[i])
        else  
            _refreshCurrency(panel_price ,data.firstBuyItem[i])
        end
    end
...
-- buyType 0 钻石 1 代币
function RechargeMain:updateRechargeItemDiamonds(item, data, buyType)
	local Label_price = TFDirector:getChildByPath(item,"Label_price")
...
function RechargeMain:updateTokenMoneyUI()
    ...
    for i, item in ipairs(items) do
        ...
        if data then
        	...
        	cell_item:hide()
        	cell_token:show()
        	self:updateRechargeItemToken(cell_token, data)
        end
    end
