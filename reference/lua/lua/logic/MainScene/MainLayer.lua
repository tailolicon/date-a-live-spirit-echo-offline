...
function MainLayer:initUI(ui)
    ...
    if  self.Image_mailTip then
        self.Image_mailTip.Label_title = TFDirector:getChildByPath(self.Button_mail, "Label_title")
    end
    self.mail_bubble = TFDirector:getChildByPath(self.Button_mail, "img_bubble")
    if self.mail_bubble then
        self.mail_bubble:hide()
    end
    ...
    if self.button_Caociyuan then
        self.button_Caociyuan:setTextureNormal("ui/activity/fanShi/08.png")
        self.button_Caociyuan:setTexturePressed("ui/activity/fanShi/08.png")
    end
	...
	if true then
        ...
        self.Button_kefu = self:createCustomBtnOnBear("ui/mainLayer/"..uipathSet.."/kefu_btn.png" , ccp(52 , 101) , "https://www.facebook.com/profile.php?id=61576733955652#" , true)
        
        self.Button_fb = self:createCustomBtnOnBear("ui/mainLayer/"..uipathSet.."/btn_x.png" , ccp(52 + 70 , 101) , "https://x.com/SpiritEcho_DAL")
        self.Button_twitter = self:createCustomBtnOnBear("ui/mainLayer/"..uipathSet.."/btn_instagram.png" , ccp(52 + 2*70 , 101) , "https://www.instagram.com/spiritecho_dal/")
    
        self.Button_dis = self:createCustomBtnOnBear("ui/mainLayer/"..uipathSet.."/btn_dis.png" , ccp(52 +3*70 , 101) , "https://discord.gg/sWFxcjPHcA")

        -- self.btn_bind = TFButton:create("ui/mainLayer/new_ui_1/bind.png")
        -- self.btn_bind:setPosition(ccp(385 , 103))
        -- self.Panel_btListEx:addChild(self.btn_bind)

        self.Button_backPlayer:setPosition(ccp(52 +4*70 , 101) )
	end
    ...
    --创建竞技场活动图标
    self.Button_arena= TFButton:create("ui/mainLayer/new_ui/btn_arena.png")
    -- self.Button_arena:hide()
    self.Image_arena_red = TFImage:create("ui/common/news_small.png")
    self.Button_arena:addChild(self.Image_arena_red)
    self.Image_arena_red:hide()
    self.Image_arena_red:setPosition(49 , 0)
    self.Button_arena:setPosition(659 , 79)
    TFDirector:getChildByPath(self.Panel_right , "Panel_clip"):addChild(self.Button_arena)
...
function MainLayer:addIPText()
    ...
    local serverInfo = ServerDataMgr:getServerData()
    if serverInfo then

        dump(serverInfo)
        local ip = serverInfo.gameServerIp
        local port = serverInfo.gameServerPort
        local groupName = ServerDataMgr:getServerGroupID()
        local serverName = ServerDataMgr:getServerName_()
...
function MainLayer:updateLeftButtons()
    ...
    --只保留客服按钮
    local tmAllBtns3    = {self.Button_kefu }
    local tmAllBtnspos3 = {ccp(52,104),ccp(52,43),ccp(165,104),ccp(265,104),ccp(165,43),ccp(265,43),ccp(355,104)}
    local idx = 1
...
function MainLayer:updateSystemChat(content,chatState)
    ...
    if chatState and chatState == EC_ChatState.TEAM then
        if content.battlename then 
            battlename =   TextDataMgr:getTextAttr(content.battlename).text  
        end 
...
--			Utils:openView("activity.LightRiddles")
    ...
    if Utils:isTemporaryChannel() then 
        self.Image_chat:setTouchEnabled(false)
        self.Image_chat:hide()
    else
        self.Image_chat:setTouchEnabled(true)
        self.Image_chat:show()
    ...
    --竞技场按钮
    self.Button_arena:onClick(function (  )
        local funcIsOpen = FunctionDataMgr:isOpen(400)
        if funcIsOpen then
            Utils:openView("fuben.FubenArenaView")
        else
...
--功能开启状态变更
function MainLayer:onFuncStateChange()
    self:onUpdateArenaBtn()
end

function MainLayer:onTaskUpdateEvent(tasks)
    self:onUpdateArenaBtn()
end

function MainLayer:onUpdateArenaBtn()
    self.Button_arena:setVisible(FunctionDataMgr:isOpen(400)) 
    if self.Button_arena:isVisible() then 
        self.Image_arena_red:setVisible(ArenaDataMgr:hasReward())
    end
end
...
--应用切换到前台
function MainLayer:onEnterForeGround()
    self:timeOut(function()
        self:checkEvent()
    end,0.5)
end
...
--ios 活动跳转事件
function MainLayer:checkEvent()
    local topLayer = AlertManager:getTopLayer()
    if topLayer then 
        print_("checkEvent topLayer: ".. tostring(topLayer.__cname))
        return 
    end
    print_("checkEvent not topLayer: ")

    if CC_TARGET_PLATFORM == CC_PLATFORM_IOS and HeitaoSdk then
        local eventId = tostring(HeitaoSdk.getevent())
        print_("iOS eventId: " ..tostring(eventId))
        if not string.isNullOrEmpty(eventId) then
            --做跳转逻辑
            HeitaoSdk.cleanevent()
            if eventId == "aabbcc0123456789" then 
                Utils:showTips("模拟打开活动页")
            elseif eventId == "6747965069" then  -- 新手登录/七日签到  
                local actIndex = ActivityDataMgr:getActIdx(EC_ActivityType.SEVENDAY)
                if actIndex ~= -1 then
                    Utils:openView("activity.SevenSignView")            
                end
            elseif eventId == "6747965800" then  -- 招募反十卡池   267
                FunctionDataMgr:jSummon(1, 267) 
            elseif eventId == "6747965898" then  -- 夏日祭美九  活动 1002
                local activityInfo = ActivityDataMgr2:getActivityInfo(1002)
                if activityInfo then 
                    FunctionDataMgr:jActivity(1002)
                end
            elseif eventId == "6749036605" then -- 召唤活动链接
                FunctionDataMgr:jSummon() 
            elseif eventId == "6749041463" then -- 魔域纷争活动链接
                ...
                if activityIds and activityIds[1] then
                    ...
                    if activityKsanInfo and serverTime >= activityKsanInfo.showStartTime and  serverTime < activityKsanInfo.showEndTime then
...
-- datealivegame://?eventid=6747965069
-- datealivegame://?eventid=6747965800
-- datealivegame://?eventid=6747965898
-- 新手登录 https://apps.apple.com/us/app/id6747107379?eventid=6747965069
-- 反十 https://apps.apple.com/us/app/id6747107379?eventid=6747965800
-- 夏日祭美九 https://apps.apple.com/us/app/id6747107379?eventid=6747965898
-- 召唤活动链接
-- https://apps.apple.com/us/app/id6747107379?eventid=6749036605
-- 魔域纷争活动链接
-- https://apps.apple.com/us/app/id6747107379?eventid=6749041463
...
function MainLayer:updateOneYearBtns()
		...
		if not self:isOneCelebrationMainLayer() then
            if  (self.Button_Activity5 and self.Button_Activity5:isVisible()) 
                ...
                or (self.Button_arena:isVisible()) then
 ...
 if self.Button_Activity5 or self.button_OneYear or self.Button_Activity7 then
            if  (self.Button_Activity6 and self.Button_Activity6:isVisible()) 
                ...
                local fixLeftPos = me.p(leftPos.x - 100,leftPos.y)
                if self.Button_Activity5 then        
                    self.Button_Activity5:setPosition(fixLeftPos)
                end 

                if self.button_OneYear then
                    self.button_OneYear:setPosition(fixLeftPos)
                end 
                
                if self.Button_Activity7 then
                    self.Button_Activity7:setPosition(fixLeftPos)
                end
            else
                if self.Button_Activity5 then
        ...
        if self.Button_Activity6 or self.button_Caociyuan or self.Button_activity2 then
            if  (self.Button_Activity5 and self.Button_Activity5:isVisible()) 
                ...
                if self.Button_Activity6 then
                    ...
                    self.Button_Activity6:setPosition(me.p(rightPos.x -50 ,rightPos.y))
                end
...
function MainLayer:checkAdsShowEnable()
    ...
    self:checkSevenSign()
    self:checkEvent()
    self:checkPlayGameLogin()
end

--检查谷歌PlayGame 授权
function MainLayer:checkPlayGameLogin()
    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
        local key = "play_game_login"
        if  Utils:getIsDayChangeBySaveData(key) then
            Utils:playGameSignIn()
            print("登录谷歌Playgame")
        end
    end
end
...
function MainLayer:onUpdateActivitysState()
    ...
    self.Button_Activity6:setVisible(#activityInfos > 0)
    if #activityInfos > 0 then 
        if activityInfos[1].activityType == 40 then 
