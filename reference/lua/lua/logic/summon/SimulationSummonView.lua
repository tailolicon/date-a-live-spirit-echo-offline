...
function SimulationSummonView:createTFLabel(parent)
	    local label = TFLabel:create()
	    parent:addChild(label)
        label:setFontName("font/fangzheng_zhunyuan.ttf")
        label:setFontSize(24)
        label:enableStroke(ccc3(89, 120, 232), 1)
        -- label:setTextAreaSize(CCSize(200 , 0))
        label:setAnchorPoint(ccp(0.5 , 0.5))
        label:setPosition(89 , -22)
        label:setFontColor(ccc3(255 , 255, 255))
        label:setText("神奇的大象")
        label:setRotation(120)
    return  label
end
...
function SimulationSummonView:initUI(ui)
	...
	for i = 1, 4 do
		...
		local _labelName = self:createTFLabel(_root)
		_foo = {}
...
function SimulationSummonView:updateSummonItem2(index)
	...
	foo.root:show()
	foo._effect:show()
	foo._labelName:setTextById(summonCfg.name2)
	foo.root:setTexture(summonCfg.firework)
	foo.lock:setVisible(not SimulationSummonDataMgr:checkIsOpenById(summonCfg.id))
    foo._lockTip:setVisible(summonCfg.unlocklevel ~= 0 or summonCfg.day ~= 0)
    if summonCfg.unlocklevel ~= 0 then 
    	foo._lockTip:setTextById(15010119,summonCfg.unlocklevel)
    elseif summonCfg.day ~= 0 then 
    	foo._lockTip:setTextById(18100029,summonCfg.day)
    end
...
function SimulationSummonView:updateSummonItem(index)
    ...
    foo.Label_lock:setVisible(summonCfg.unlocklevel ~= 0 or summonCfg.day ~= 0)
    if summonCfg.unlocklevel ~= 0 then 
    	foo.Label_lock:setTextById(15010119,summonCfg.unlocklevel)
    elseif summonCfg.day ~= 0 then 
    	foo.Label_lock:setTextById(18100029,summonCfg.day)
	end
