...
function SummonContractDetailsView:updateView()
    ...
    local  data= RechargeDataMgr:getOneRechargeCfg(cfg.SellingPrice)
    if data.buyType == 0 then  
        self.Label_build:setTextById(32332,data.rechargeCfg.price*0.01)  --800007,
        self.Button_build:getChildByName("Image_icon"):hide()
    else
        local rechargeCost = data.exchangeCost[1]
