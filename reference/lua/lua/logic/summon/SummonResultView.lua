...
function SummonResultView:initData(reward, isShieldEffect)
    ...
    self.openReward_ = {}
    self.showAnimation =  SettingDataMgr:getSummonAnimation() == 1
end
...
function SummonResultView:initUI(ui)
    ...
    self.Label_animation_tip = TFDirector:getChildByPath(self.Panel_result, "Label_animation_tip")
    self.Label_animation_tip:setTextById(18000332)
    ...
    self.Button_open:setVisible(self.showAnimation)
...
function SummonResultView:refreshView()
    ...
    if self.showAnimation then --TODO 显示招募动画
...
function SummonResultView:turnOnReward(index)
    ...
    --根据品质来判断显示不显示特效
    local bornEffect = itemCfg.bornEffect and (self.showAnimation or ((itemCfg.rarity or 0) > 4))   -- itemCfg.bornEffect
    self.Panel_mask_single:setVisible(bornEffect)
...
function SummonResultView:isHighQuality(index)
    ...
    elseif itemCfg.superType == EC_ResourceType.REWARD then
        isHighQuality = itemCfg.quality >= EC_ItemQuality.BLUE
    elseif itemCfg.superType == EC_ResourceType.PET then    
        isHighQuality = itemCfg.quality >= EC_ItemQuality.PURPLE
    end
