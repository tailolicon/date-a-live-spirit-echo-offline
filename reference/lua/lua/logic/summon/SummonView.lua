...
function SummonView:initUI(ui)
    ...
    --宠物召唤
    self.Panel_pet = TFDirector:getChildByPath(self.Panel_root, "Panel_pet"):hide()
    self.Button_winsh_card = TFDirector:getChildByPath(self.Panel_pet, "Button_winsh_card"):hide()
    self.Image_wish_item   = TFDirector:getChildByPath(self.Button_winsh_card, "Image_wish_item"):hide()
    local Image_pet_Count = TFDirector:getChildByPath(self.Panel_pet, "Image_pet_Count")
    self.Label_pet_tip1 = TFDirector:getChildByPath(Image_pet_Count, "Label_pet_tip1"):hide()
    self.Label_pet_tip2 = TFDirector:getChildByPath(Image_pet_Count, "Label_pet_tip2")
    self.Label_pet_Count = TFDirector:getChildByPath(Image_pet_Count, "Label_pet_Count")
    self.Label_wish_card = TFDirector:getChildByPath(self.Button_winsh_card, "Label_wish_card")
    self.Label_wish_card:setTextById(13300263)
...
function SummonView:updateSelectInfo()
    ...
    if summonInfo and (summonCfg.summonType == EC_SummonType.APPOINT_HERO or summonCfg.summonType == EC_SummonType.CONTRACT_ROLE) then
    ...
    if summonInfo and (summonCfg.summonType == EC_SummonType.APPOINT_EQUIPMENT or summonCfg.summonType == EC_SummonType.CONTRACT_EQUIP) then
    ...
    or summonCfg.summonType == EC_SummonType.PET
    ...
    elseif summonCfg.summonType == EC_SummonType.CONTRACT_ROLE or summonCfg.summonType == EC_SummonType.CONTRACT_EQUIP then 
        local goods = GoodsDataMgr:getItem(summonCfg.ext.lockItem) 
        if goods then 
            dump(goods)
            local _,goodsData = next(goods)
            local remainTime  = math.max(0, goodsData.outTime - ServerDataMgr:getServerTime())
            if remainTime > 0 then
                ...
                -- if hour ~="00" then
                --     self.Label_tips:setTextById(13100053, hour, min)
                -- else
                --     self.Label_tips:setTextById(13100054, min, sec)
                -- end
            end
        end
    end
    ...
    --宠物召唤
    self.Panel_pet:setVisible(summonType == EC_SummonType.PET)
    if summonType == EC_SummonType.PET then 
        -- self.Label_pet_tip1:setTextById(1200066)
        self.Label_pet_tip2:setTextById(27600)
        local num = SummonDataMgr:getPetSummonRemainCount()
        self.Label_pet_Count:setText(num)
        -- self.Image_ad:setTexture(summonCfg.icon)
        local configs = SummonDataMgr:getPetWishCard(summonCfg.id)
        --显示不显示心愿卡选择按钮
        self.Button_winsh_card:setVisible(#configs > 0)

        local wishPool = SummonDataMgr:getWishSummonPool(summonCfg.id)
        if wishPool > 0 then 
            local summonPoolCfg = TabDataMgr:getData("SummonPool" ,wishPool)
            for k,v in pairs(summonPoolCfg.itemMap) do
                local petCfg = PetDataMgr:getPetCfg(k)  
                if petCfg then 
                    self.Image_wish_item:setTexture(petCfg.icon)
                    break 
                end
            end
            self.Image_wish_item:show()
        else
            self.Image_wish_item:hide()
        end
    end
...
function SummonView:registerEvents()
    ...
    self.Button_award:onClick(function()
        local summon = self.viewSelectSummon_
        local summonCfg = SummonDataMgr:getSummonCfg(summon[1].id)
        if summonCfg.summonType == EC_SummonType.APPOINT_HERO or summonCfg.summonType == EC_SummonType.CONTRACT_ROLE then
            local isInStage = SummonDataMgr:isInOpenStage(summon[1].id)
            if not isInStage then
                Utils:showTips(1200054)
                return
    ...
    self.Button_equip:onClick(function()
        local summon = self.viewSelectSummon_
        local summonCfg = SummonDataMgr:getSummonCfg(summon[1].id)
        if summonCfg.summonType == EC_SummonType.APPOINT_EQUIPMENT or summonCfg.summonType == EC_SummonType.CONTRACT_EQUIP then
            local isInStage = SummonDataMgr:isInOpenStage(summon[1].id)
            if not isInStage then
                Utils:showTips(1200054)
                return
    ...
    self.Button_winsh_card:onClick(function() 
        local summon = self.viewSelectSummon_
        local summonCfg = SummonDataMgr:getSummonCfg(summon[1].id)
        Utils:openView("summon.SummonPetView",summonCfg.id)
