...
function CollectMainView:initUI(ui)
	...
	self.pet_panel = root_panel:getChildByName("Panel_enter_pet")
	local Label_collect_title = self.pet_panel:getChildByName("Label_collect_title")
	Label_collect_title:setTextById(1706461)
...
function CollectMainView:updateCollectPet() --宠物
	self:commonUpdateInfo(self.pet_panel,EC_CollectPage.PET)
end
...
function CollectMainView:registerEvents()
	...
	self.pet_panel:onClick(function()
		if CollectDataMgr:checkPageCanEnter(EC_CollectPage.PET) == false then
			Utils:showTips(241009)
			return
		end
		Utils:openView("collect.CollectPetView")
	end)
