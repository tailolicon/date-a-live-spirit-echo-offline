...
--单独节点适配
function BaseLayer:ResolutionAdaptationNode(node,containChild)
    ...
    if containChild then
        local childs = node:getChildren()
        if #childs ~= 0 then
            for i, child in pairs(childs) do
                self:ResolutionAdaptationNode(child,containChild)
            end
        end
    end
end
...
function BaseLayer:ResolutionAdaptation()
    self:ResolutionAdaptationNode(self.ui,true)
end
