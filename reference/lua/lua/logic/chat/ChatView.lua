...
function ChatView:refreshPrefabe()
    ...
    self.prefabe.maxWidth      = self.prefabe:getSize().width - 100 -10
    ...
    self.prefabe.textMaxWidth = self.prefabe:getSize().width - 140 -10
end
...
function ChatView:updateItem(item, chatInfo)
    ...
    if not string.isNullOrEmpty(chatInfo.tContent) then 
        content = content  .."\n#separator#\n" .. chatInfo.tContent 
    end
    ...
    local Button_translate = nil
    if img_content then 
        img_content:setVisible(true)
        Button_translate = img_content:getChildByName("Button_translate")
    end
    if Button_translate then 
        Button_translate:hide()
        if chatInfo.channel == EC_ChatType.WORLD or chatInfo.channel == EC_ChatType.PRIVATE or chatInfo.channel == EC_ChatType.TEAM then
            if not isSelf then 
                Button_translate:setVisible(not chatInfo.tState or  chatInfo.tState < 2 )
                Button_translate:onClick(function()
                    ChatDataMgr:translate(chatInfo)
                end)
            end
        end
    ...
    if chatInfo.fun and (chatInfo.fun == EC_ChatState.AI_ROBOT or chatInfo.fun == EC_ChatState.SYSTEM or chatInfo.fun == EC_ChatState.AI_ADVICE) then
        ...
        if image_head and heroCid and heroCid ~= 0 then
            ...
            if Button_translate and img_content then 
                local _PercentX  = img_content:getScaleX() < 0 and  0 or 100
                local _PositionX = img_content:getScaleX() < 0 and  0 or 200
                local UILayoutViewModel = 
                {
                ...
                Button_translate:setLayoutByTable(UILayoutViewModel)
            end
...
function ChatView:setRichContent(lab_content,str,fontColor,isSelf)
    ...
    str = string.gsub(str, "#separator#", "<img src='ui/chat/separator.png' size='340x30'></img>")
...
function ChatView:registerEvents()
    --翻译回调
    EventMgr:addEventListener(self,EV_RECV_TRANSLATE, handler(self.onTranslateCallBack, self))
...
function ChatView:onTranslateCallBack()
