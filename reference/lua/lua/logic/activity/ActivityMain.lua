...
function ActivityMain:initUI(ui)
    ...
    if not self.Panel_rank then 
        self.Panel_rank =  self.Panel_tzr:clone()
        self.Panel_root:addChild(self.Panel_rank)
    end
    self.Panel_rank:hide()
    --七日累充
    self.Panel_voucherSevenDay = TFDirector:getChildByPath(self.Panel_root, "Panel_voucherSevenDay")
    if not self.Panel_voucherSevenDay then 
        self.Panel_voucherSevenDay =  self.Panel_tzr:clone()
        self.Panel_root:addChild(self.Panel_voucherSevenDay)
    end
    self.Panel_voucherSevenDay:hide()
    ...
    self.Panel_firstCharge    = TFDirector:getChildByPath(ui,"Panel_firstCharge"):hide()
    self.actPanels = {
        ...
        [EC_ActivityType.LUXURY_FIRST_CHARGE] = self.Panel_firstCharge, --豪华首充

        [EC_ActivityType.RANK2026] = self.Panel_rank, --打榜功能
        [EC_ActivityType.VOUCHER_SEVEN_DAY] = self.Panel_voucherSevenDay, --七日累充
    }
    ...
    self.actUpdateFun = {
        ...
        [EC_ActivityType.LUXURY_FIRST_CHARGE] = self.updateFirstCharge,

        [EC_ActivityType.RANK2026] = self.update2026Rank, --打榜功能功能更新
        [EC_ActivityType.VOUCHER_SEVEN_DAY] = self.updateVoucherSevenDay, --七日累充
    }
...
function ActivityMain:initActivitysBtn()
    ...
    if giftData  and cnt < giftData.buyCount then 
        local Panel_cardBtn = self.actBtnItem:clone()
        local Button = TFDirector:getChildByName(Panel_cardBtn, "Button")
        local Label = TFDirector:getChildByName(Button, "Label")
        local Image_icon = TFDirector:getChildByName(Button, "Image_icon")
        local RedTip = TFDirector:getChildByName(Panel_cardBtn, "RedTip")
        Label:setTextById(18000302)
        Image_icon:setTexture("ui/activity/activity_score/010.png")
        self.btnListView:pushBackCustomItem(Panel_cardBtn)
        self.actBtns[EC_ActivityType.LUXURY_FIRST_CHARGE] = Button
        Button:onClick(function()

            self:onTouchActBtn(Button)
        end)
        Button.id = EC_ActivityType.LUXURY_FIRST_CHARGE
    end
    ...
    if rankOpen then 
        ...
        local Image_icon = TFDirector:getChildByName(Button, "Image_icon")
        local RedTip = TFDirector:getChildByName(Panel_cardBtn, "RedTip")
        Label:setTextById(2600036)
        Image_icon:setTexture("ui/activity/2026/010.png")
        self.btnListView:pushBackCustomItem(Panel_cardBtn)
        self.actBtns[EC_ActivityType.RANK2026] = Button
        Button:onClick(function()

            self:onTouchActBtn(Button)
        end)
        Button.id = EC_ActivityType.RANK2026
    end


    local voucherSevenDayData = ActivityDataMgr:getVoucherSevenDayData()
    if voucherSevenDayData and voucherSevenDayData.nextOpenTime and voucherSevenDayData.nextOpenTime > 0 then
        local Panel_cardBtn = self.actBtnItem:clone()
        local Button = TFDirector:getChildByName(Panel_cardBtn, "Button")
        local Label = TFDirector:getChildByName(Button, "Label")
        local Image_icon = TFDirector:getChildByName(Button, "Image_icon")
        local RedTip = TFDirector:getChildByName(Panel_cardBtn, "RedTip")
        Label:setTextById(2600013)
        Image_icon:setTexture("ui/store/new_ui/icon1.png")
        self.btnListView:pushBackCustomItem(Panel_cardBtn)
        self.actBtns[EC_ActivityType.VOUCHER_SEVEN_DAY] = Button
        Button:onClick(function()

            self:onTouchActBtn(Button)
        end)
        Button.id = EC_ActivityType.VOUCHER_SEVEN_DAY
    end
...
function ActivityMain:showRedPoint()
    for k,v in pairs(self.actBtns) do
        ...
        elseif v.id == EC_ActivityType.LUXURY_FIRST_CHARGE then --豪华首充
            local giftData = RechargeDataMgr:getGiftSingleData(101)
            if giftData  then 
                local cnt =  RechargeDataMgr:getBuyCount(101)
                if cnt < giftData.buyCount then 
                    redTip:setVisible(not GlobalVarDataMgr:getValue(GV_LUXURY_RECHARGE))
                else
                    redTip:setVisible(false)
                end
            else
...
function ActivityMain:onTouchActBtn(button)
    ...
    if not self.actPanels[button.id] then
        Box("不支持的活动ID:"..tostring(button.id))
        return
    end
...
function ActivityMain:updateSevenEx()
    ...
    local daibiImg = TFDirector:getChildByPath(self.panel_sevenEx,"img_daibi"):hide()  
    local sevenGiftBag = RechargeDataMgr:getSevenGiftBag(self.selectSevenExIdx)
    if sevenGiftBag then
        if sevenGiftBag.buyType == 1 then  
            ...
            Label_price_now:setTextById(32332 ,sevenGiftBag.rechargeCfg.price*0.01)
            Label_price_now:setPositionX(-45)
        end
    ...
    Label_price_old:setTextById(32332,curDayCfg.saleprice[500002])
...
function ActivityMain:updateFirstCharge()



    if not self.Panel_firstCharge.inited then 
        self.Panel_firstCharge.inited = true
        local Image_bg = TFDirector:getChildByPath(self.Panel_firstCharge, "Image_bg")
        local Button_recharge = TFDirector:getChildByPath(Image_bg, "Button_recharge")
        local Label_price = TFDirector:getChildByPath(Image_bg, "Label_price")
        local Label_price_new = TFDirector:getChildByPath(Button_recharge, "Label_price_new")
        local Image_discount = TFDirector:getChildByPath(Image_bg, "Image_discount")
        -- Utils:setDiscountTexture(Image_discount)
        -- local newGiftBag = TabDataMgr:getData("Recharge")[101]
        -- local giftBagCfg = TabDataMgr:getData("RechargeGiftBag")[newGiftBag.goodsId]
        self.Panel_firstCharge.Button_recharge = Button_recharge
        Button_recharge:setGrayEnabled(true)
        Button_recharge:setTouchEnabled(false)
        local giftData = RechargeDataMgr:getGiftSingleData(101)
        ...
        --充值
        Button_recharge:onClick(function ( ... )
            RechargeDataMgr:getOrderNO(101)
            --print("充值>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        end)

        --Label_price:setTextById(32331,tostring(giftData.rechargeCfg.price))
        Label_price:hide()
        Label_price_new:setTextById(32332,tostring(giftData.des2)) --折后价格
    ...
    local cnt =  RechargeDataMgr:getBuyCount(101)
    if giftData and (cnt < giftData.buyCount) then
        self.Panel_firstCharge.Button_recharge:setGrayEnabled(false)
        self.Panel_firstCharge.Button_recharge:setTouchEnabled(true)
    else
        self.Panel_firstCharge.Button_recharge:setGrayEnabled(true)
        self.Panel_firstCharge.Button_recharge:setTouchEnabled(false)
    end
...
function ActivityMain:updateVoucherSevenDay()
    if not self.voucherSevenDayView_ then
        self.voucherSevenDayView_ =  requireNew("lua.logic.activity.VoucherSevenDayView"):new()
        local x = -GameConfig.WS.width * 0.5
        local y = -GameConfig.WS.height * 0.5
        local size = self.voucherSevenDayView_:getSize()
        self.voucherSevenDayView_:setPosition(ccp(x-32, y))
        self:addLayerToNode(self.voucherSevenDayView_, self.Panel_voucherSevenDay)
    end
end


--打榜功能更新
function ActivityMain:update2026Rank()
    if not self.rank2026View_ then
        self.rank2026View_ = requireNew("lua.logic.activity.Rank2026View"):new(true)
        local x = -GameConfig.WS.width * 0.5
        local y = -GameConfig.WS.height * 0.5
        local size = self.rank2026View_:getSize()
        self.rank2026View_:setPosition(ccp(x-32, y))
        self:addLayerToNode(self.rank2026View_, self.Panel_rank)
    end
end
