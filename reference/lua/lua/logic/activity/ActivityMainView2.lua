...
function addActivityItem(self)
    ...
    Panel_activityItem.setTabBtnIcon = function (self,activity)
        ...
        --TODO 文字根据图片做自适应
        local _size =  self.ImageNormal:getContentSize()
        local _pos  = self.ImageSelect.activityName:getPosition()
        _pos.x = _size.width * 0.5 - 5   - 25
        _pos.y = _size.height* 0.5 * -1 + 15
        self.ImageSelect.activityName:setPosition(_pos)
        self.ImageNormal.activityName:setPosition(_pos)
    end
