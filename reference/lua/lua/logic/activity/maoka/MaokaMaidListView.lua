...
function MaokaMaidListView:ctor( ... )
	...
	self:init("lua.uiconfig.secondary.uiconfig_zn.activity.maidList")
end
...
function MaokaMaidListView:initUI( ui )
    ...
    self.Label_1:setTextById(13316892) --当前拥有
	
	self.Label_tip = TFDirector:getChildByPath(self.Panel_base,"Label_tip")
    self.Label_tip:setTextById(13316911)-- 总提升

	self.Label_addTip = TFDirector:getChildByPath(self.Panel_base,"Label_addTip")
    self.Label_addTip:setTextById(13316913)-- 获得女仆并且激活配方才可让加成生效

	self.Label_btn = TFDirector:getChildByPath(self.Panel_base,"Label_btn")
    self.Label_btn:setTextById(13316912)-- 设置看板
...
function MaokaMaidListView:updateItem( item, data )
	...
	local Image_using_Label_cattery_1 = TFDirector:getChildByPath(Image_using,"Label_cattery_1")
	local Image_noHave_Label_cattery_1 = TFDirector:getChildByPath(Image_noHave,"Label_cattery_1")
	Image_using_Label_cattery_1:setTextById(2108055) --使用中
