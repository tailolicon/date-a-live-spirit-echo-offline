...
function MaokaAdventureView:ctor( ... )
	...
	self:init("lua.uiconfig.secondary.uiconfig_zn.activity.adventure")
end
...
function MaokaAdventureView:initUI( ui )
	...
	self.Button_get_name = TFDirector:getChildByPath(self.Button_get,"Label_btn")
	self.BButton_go_name = TFDirector:getChildByPath(self.Button_go,"Label_btn")
	self.Label_time_Label_tip = TFDirector:getChildByPath(self.Label_time,"Label_tip")
    self.Label_time_Label_tip:setTextById(13317144) --倒计时:
    self.BButton_go_name:setTextById(13410017)
    self.Button_get_name:setTextById(1820002)
...
function MaokaAdventureView:updateLeftPanel( ... )
	...
	if not self.initLeftPanel then
		...
		for i = 1,10 do
			...
			if i<=5 then 
					local Label_name = TFDirector:getChildByPath(self.Panel_pos[i],"Label_name")
					Label_name:setTextById(PANEL_NAME_IDS[i])
			end
		end
	end
...
function MaokaAdventureView:updateRightPanel( ... )
	...
	if not self.initRightPanel then
		...
		self.Label_tip = TFDirector:getChildByPath(self.Panel_right,"Label_tip")
		self.Label_tip:setTextById(13310002)
	end
	if not self.curTaskId then return end
	local taskInfo ,progressInfo = MaokaActivityMgr:getActivityItemInfo(self.curTaskId)

	self.Label_name:setTextById(taskInfo.details)
	self.Label_taskDes:setTextById(taskInfo.extendData.des2)
