...
function MaokaFormulaMakeView:ctor( formulaId, etime)
	...
	self.etime = etime
	self:resetData()
	self:init("lua.uiconfig.secondary.uiconfig_zn.activity.formulaMake")
end
...
function MaokaFormulaMakeView:initUI( ui )
	...
	self.Image_make_Label_tip = TFDirector:getChildByPath(self.Image_make,"Label_tip")
    self.Image_make_Label_tip:setTextById(13317150) -- 放入任意四种材料，尝试合成配方
...
function MaokaFormulaMakeView:updateRightPanel( ... )
	...
	if not self.initRightPanel then
		...
		self.Button_make_Label_btn = TFDirector:getChildByPath(self.Button_make,"Label_btn")
		self.Button_make_Label_btn:setTextById(13317152) --开始调制
	end
...
function MaokaFormulaMakeView:updateUrgencePanel( ... )
	...
	if not self.initUrgencePanel then
	    ...
	    local Label_1 = TFDirector:getChildByPath(self.Panel_urgence,"Label_1")
	    Label_1:setTextById(13317151) --紧急制作需求：
        local Label_tip = TFDirector:getChildByPath(self.Panel_urgence,"Label_tip")
	    Label_tip:setTextById(13317144) --倒计时
	end
