...
function SevenExGiftView:initUI(ui)
    ...
    local img_daibi = TFDirector:getChildByPath(self.Button_sEx_buy,"img_daibi")
    if self.sevenGiftBag.buyType == 1 then 
        Label_price_now:setString(self.sevenGiftBag.exchangeCost[1].num)
        img_daibi:show()
    else
        img_daibi:hide()
        Label_price_now:setAnchorPoint(ccp(0.5,0.5))
        Label_price_now:setPosition(ccp(0,0))
        Label_price_now:setTextById(32332,self.sevenGiftBag.rechargeCfg.price*0.01)
    end
