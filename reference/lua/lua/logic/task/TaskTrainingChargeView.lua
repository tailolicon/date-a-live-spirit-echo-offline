...
function TaskTrainingChargeView:initUI(ui)
    ...
    self.Label_price1.oldPos = self.Label_price1:getPosition()
    self.Label_price2.oldPos = self.Label_price2:getPosition()
...
function TaskTrainingChargeView:refreshUI()
    ...
    if chargeCfg1.buyType == 1 then 
        local exchangeCfg1 =  GoodsDataMgr:getItemCfg(chargeCfg1.exchangeCost[1].id);
        ...
        self.Label_price1:setPositionX(self.Label_price1.oldPos.x)
    else

        self.Label_price1:getChildByName("imgToken"):setVisible(false)
        self.Label_price1:setTextById(32332,chargeCfg1.rechargeCfg.price/100)
        self.Label_price1:setPositionX(self.Label_price1.oldPos.x - 20)
    end
    ...
    if chargeCfg2.buyType == 1 then 
        local exchangeCfg2 =  GoodsDataMgr:getItemCfg(chargeCfg2.exchangeCost[1].id);
        self.Label_price2:setText(chargeCfg2.exchangeCost[1].num)
        self.Label_price2:getChildByName("imgToken"):setTexture(exchangeCfg2.icon);
        self.Label_price2:getChildByName("imgToken"):setSize(CCSizeMake(30,30));
        self.Label_price2:setPositionX(self.Label_price2.oldPos.x)
    else

        self.Label_price2:getChildByName("imgToken"):setVisible(false)
        self.Label_price2:setTextById(32332,chargeCfg2.rechargeCfg.price/100)
        self.Label_price2:setPositionX(self.Label_price2.oldPos.x - 20)
    end
    ...
    elseif state == 1 then
        ...
        if chargeCfg3.buyType == 1 then 
            self.Label_price2:setText(chargeCfg3.exchangeCost[1].num)
        else
            self.Label_price2:setTextById(32332,chargeCfg3.rechargeCfg.price/100)
        end
