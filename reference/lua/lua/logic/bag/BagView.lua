...
function  BagView:initTableView( )
    
    self.tableView = Utils:scrollView2TableView(self.ScrollView_item)
    ...
    --滚动条
    self.Image_scrollBarModel:show()
    local Image_scrollBarInner = TFDirector:getChildByPath(self.Image_scrollBarModel, "Image_scrollBarInner")
    self.scrollBar = UIScrollBar:create(self.Image_scrollBarModel, Image_scrollBarInner)


end

function BagView:reloadData(keep)
    local contentOffset = self.tableView:getContentOffset()
    self.tableView:reloadData()
    -- if keep then 
    --             --     contentOffset.y = contentOffset.y - self.fixheight
    --     self.tableView:setContentOffset(contentOffset)
    -- end
end


function BagView:tableScroll(tableView)
...
function BagView:tableCellSize(tableView, idx)
    local tableSize = tableView:getContentSize()
    local itemSize  = self.Panel_headItem:getContentSize()
    return  itemSize.height+2 ,  tableSize.width
end

function BagView:numberOfCells( tableView )
    local size = #self.viewDatas  
    return math.ceil(size/7)
end

-- function BagView:getData()
--     -- body
-- end


function BagView:tableCellAtIndex(tableView,idx)

    local tableSize = tableView:getContentSize()
    local _itemW  =  tableSize.width /7
    ...
    if nil == cell then
        ...
        for i=1,7 do
            ...
            item.Image_limit = TFDirector:getChildByPath(item.root, "Image_limit")
            item.Label_time = TFDirector:getChildByPath(item.root, "Label_time")
            item.Panel_aging = TFDirector:getChildByPath(item.root, "Panel_aging")
            item.Image_new = TFDirector:getChildByPath(item.root, "Image_new")
            ...
            item.Label_aging = TFDirector:getChildByPath(item.Image_aging, "Label_aging")
            item.Image_carry = TFDirector:getChildByPath(item.root, "Image_carry")
            item.trail_flag = TFDirector:getChildByPath(item.root, "trail_flag")
            item.Panel_pos = TFDirector:getChildByPath(item.root, "Panel_pos"):hide()

            --限时图标
            local limit_icon = TFImage:create("ui/bag/new_ui/limit.png");
            item.root:addChild(limit_icon,999)
            limit_icon:setPosition(ccp(30,40))
            limit_icon:hide()
            item.limit_icon = limit_icon

            local Panel_goodsItem = PrefabDataMgr:getPrefab("Panel_goodsItem"):clone()
            Panel_goodsItem:Pos(0, 0)
            Panel_goodsItem:AddTo(item.Panel_head)
            item.Panel_goodsItem = Panel_goodsItem
            ...
            item.idx = i-1
            item:show()
            item:setPosition(ccp(tableSize.width / -2  - _itemW/2 + _itemW * i,-itemSize.height))
            cell:addChild(item)
            cell.items[i]  =item   
        end
...
function BagView:updateCellItem(cell,idx)
    for i=1,7 do
       local item = cell.items[i]

       self:onUpdateItem(item , idx*7 + i)
       --item:setVisible(i<5)
    end
end
...
function BagView:updateBag(index)
    ...
    self.viewDatas = self.goodsData_[self.selectIndex_]
    self:reloadData()
    self:updateCapacity()
end
...
function BagView:onUpdateItem(item, idx)
    local data =  self.viewDatas[idx]
    if not data then 
        item:hide() 
        return
    end 
    ...
    if self.isSellModel_ then 
        foo.Image_select:setVisible(self.sellSelectData_[data.id] ~= nil )
    else
            ...
            if sender:hitTest(touchPos) then
                ...
                if self.isShowSubMenu then
                    ...
                    if eventName == "ended" then
                        local moveH = math.abs(touchPos.y - beganPos.y)
                        --print("moveH:"..moveH)
                        if moveH < 50 then --垂直方向移动小于50 才响应点击
...
function BagView:selectGoodsItem(item, data)
    ...
    if btnConfig.sellType == EC_BagSellType.MULTISELECT then
        ...
        if self.sellSelectData_[data.id] then 
            self.sellSelectData_[data.id] = nil
        else
            self.sellSelectData_[data.id] = {
                num = 1,
                data = data,
            }
        end
...
function BagView:singleSellOp(isAddOp)
    for i, v in pairs(self.sellSelectData_) do
        if isAddOp then
...
function BagView:onCountDownPer()
    ...
    if #self.viewDatas  > 0 then
        for i,v in ipairs(self.viewDatas) do
...
function BagView:isCanSellAction()
    local rets =   table.count(self.sellSelectData_) > 0
    return rets
end
...
function BagView:updateOutTime(item, data)
    ...
    item.Image_limit:hide()

    if not failure then
        local _, hour, min = Utils:getFuzzyDHMS(remainTime, true)
        item.Label_time:setTextById(301011, hour, min)
    end
end
