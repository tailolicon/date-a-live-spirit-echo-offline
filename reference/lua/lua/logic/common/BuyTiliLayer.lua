...
function BuyTiliLayer:updateUI()
    ...
    local priceIndex = math.min(#self.prices, buyCount + 1)
    self.cost = self.prices[priceIndex][1]
    ...
    self.Label_costNum:setText(self.cost[1].num.."aaa")
...
function BuyTiliLayer:updateUseItems()
    ...
    self.Label_tips:setTextById("r304006", name, remainCount)
    local priceIndex = math.min(#self.prices, buyCount + 1)
    local cost = self.prices[priceIndex][1]
    self.Label_costNum:setText(cost[1].num)

    local isGray = false
    if remainCount <= 0 then
        isGray = true
    end
    self.Button_ok:setGrayEnabled(isGray)
    self.Button_ok:setTouchEnabled(not isGray)
