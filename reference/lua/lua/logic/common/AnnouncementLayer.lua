...
function AnnouncementLayer:getAnnouncementInfo( ... )
   print(self.announcementUrl[self.urlIdx])
    ...
    --     print("上报日志成功")
    -- end
   -- HttpHelper:get("https://datealive.com",handler(self.updateNoticeData, self))

    --HttpHelper:post("http://170.106.160.44:7070/globalNotice/get_global_notice", "",handler(self.updateNoticeData, self))


    --HttpHelper:post("https://43.130.171.121:8082/globalNotice/get_global_notice", "",handler(self.updateNoticeData, self))
