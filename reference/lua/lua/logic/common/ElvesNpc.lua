...
function TTFLive2D:ctor(roleInfo,isMainLayer)
	...
	if not self.setEyeBlinkEnabled then 
		self.setEyeBlinkEnabled = function () end
	end
	if not self.getCurrentEffectID then 
		self.getCurrentEffectID = function () end
	end

	if not self.getCurrentEffectTime then 
		self.getCurrentEffectTime = function () end
	end
end
...
function TTFLive2D:playVoice(voice)
    if self.effectHandle and self.effectHandle ~= -1 then
...
function TTFLive2D:setVoiceVolume(soundValue)
    ...
    if not string.isNullOrEmpty(voicePath) and string.match(voicePath ,"%.mp3$") and AudioEngine.setPathVolume then
    	-- Box("voicePath:"..tostring(soundValue))
        local baseVol = SettingDataMgr:getSoundMainVal()
        local voiceVol = SettingDataMgr:getSoundVoiceVal()
