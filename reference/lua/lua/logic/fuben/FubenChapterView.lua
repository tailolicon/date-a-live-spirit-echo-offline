...
function FubenChapterView:initData(fubenType, selectChapter, theaterId)
    ...
    if Utils:isTemporaryChannel() then
        local _data = activity.data
        activity.data = {}
        for i,v in ipairs(_data) do
            if v ~= 401 and v ~= 402 and v~= 409 then 
                table.insert(activity.data,v)
            end
        end
    end
...
function FubenChapterView:updateActivityTiming(index, chapterCid)
    ...
    elseif chapterCid == EC_ActivityFubenType.ARENA then
       self.activityChapterState_[index] = false
        foo.Panel_timing:hide()

    elseif chapterCid == EC_ActivityFubenType.ELEMENT_TRIAL then
        -- print("------------------------"..ServerDataMgr:getServerTime())
        self.activityChapterState_[index] = true
        local elementTrailData = ActivityDataMgr:getActivity3Data(EC_ActivityFubenType.ELEMENT_TRIAL)
        --TODO测试时间
        -- elementTrailData = { startTime = 1776222884 , endTime =  (1776222884 + 1000) }
        if elementTrailData then 

            local serverTime = ServerDataMgr:getServerTime()
            if serverTime >= elementTrailData.startTime and serverTime <= elementTrailData.endTime then 
                local remainTime = math.max(0, elementTrailData.endTime - serverTime)
                local day, hour, min = Utils:getFuzzyDHMS(remainTime, true)
                if day ~= "00" then
                    foo.Label_timing:setTextById("r80002", day, hour)
                    foo.Label_tips:setTextById("r80011", day, hour)
            ...
            self.activityChapterState_[index] = false
            foo.Image_state:show()
            foo.Label_state:setTextById(300128)
        end
    end
        ...
        if subIndex == selectIndex then
            if unlock then
                ...
                local function enterActivityFuben()
                    ...
                    elseif chapterCid == EC_ActivityFubenType.ARENA then
                        local funcIsOpen = FunctionDataMgr:checkFuncOpen(400)    --  ArenaDataMgr:getArenaData() ~= nil  --FunctionDataMgr:checkFuncOpen(157)
                        if funcIsOpen then
                    ...
                    elseif chapterCid == EC_ActivityFubenType.ELEMENT_TRIAL then
                        if ActivityDataMgr:isActivity3Open(EC_ActivityFubenType.ELEMENT_TRIAL) then 
                            Utils:openView("elementTrail.ElementTrailMainView")
                        else
                            Utils:showTips(300128)
                        end
