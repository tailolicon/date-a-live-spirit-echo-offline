...
function FubenSquadView:initAssistanData()
    ...
    local heros = FubenDataMgr:getAssistHeros()
    if #heros > 0 then
...
function FubenSquadView:initActivityData(chapterCid, ...)
    ...
    elseif chapterCid == EC_ActivityFubenType.SNOW_FESVITAL then
        ...
        self.levelCfg_ = FubenDataMgr:getLevelCfg(self.levelCid_)
        self:initFormationData()
    elseif chapterCid == EC_ActivityFubenType.ELEMENT_TRIAL then --元素试炼
        --TODO 元素试炼
        
        local prams = {...}

        self.levelCid_ = prams[1] --关卡ID
        self.elementTrainDungeonId = prams[2] 
        self.levelCfg_ = FubenDataMgr:getLevelCfg(self.levelCid_)
        self.elementTrainDungeonCfg  = TabDataMgr:getData("ElementTrainDungeon" ,self.elementTrainDungeonId)
        self:initFormationData()

    end
end
function FubenSquadView:checkElementTrainStaminaHero(heroId)
    if self.elementTrainDungeonCfg then 
        local staminaHeroType =  self.elementTrainDungeonCfg.staminaHeroType or {}
        local heroCfg = HeroDataMgr:getHero(heroId)
        for k,v in pairs(staminaHeroType) do
            if v == heroCfg.attrType then 
                return true
            end
        end
    end
...
function FubenSquadView:initUI(ui)
    ...
    for i = 1, 3 do
        ...
        item.Panel_element_trail = TFDirector:getChildByPath(item.Panel_role, "Panel_element_trail"):hide()
        item.LoadingBar_tired_element_trail = TFDirector:getChildByPath(item.Panel_element_trail, "LoadingBar_tired")
        item.Image_up_element_trail = TFDirector:getChildByPath(item.Panel_element_trail, "Image_up"):hide()
        item.Label_tired_element_trail = TFDirector:getChildByPath(item.Panel_element_trail, "Label_tired"):hide()
        item.Panel_ksan_coin = TFDirector:getChildByPath(item.Panel_role, "Panel_ksan_coin"):hide()
        if self.fubenType_ == EC_FBType.KSAN_FUBEN then
    ...
    --元素试炼
    self.Panel_ElementTrail = TFDirector:getChildByPath(self.Panel_root, "Panel_ElementTrail"):hide()
...
function FubenSquadView:updateElementTrail()

end

--显示元素试炼
function FubenSquadView:showElementTrail()
    self.Panel_ElementTrail:show()

    local elementTrailCfg  = TabDataMgr:getData("ElementTrainDungeonLevel" ,self.levelCid_)


    --关卡属性
    self.Label_element_trail_desc_title = TFDirector:getChildByPath(self.Panel_ElementTrail, "Label_element_trail_desc_title")
    --可能掉落
    self.Label_award_title = TFDirector:getChildByPath(self.Panel_ElementTrail, "Label_award_title")
    --关卡描述
    self.Label_element_trail_buff_title = TFDirector:getChildByPath(self.Panel_ElementTrail, "Label_element_trail_buff_title")
        
    self.Label_element_trail_desc_title:setTextById(320010)
    self.Label_award_title:setTextById(300129)
    self.Label_element_trail_buff_title:setTextById(320009)


    self.Label_element_trail_desc = TFDirector:getChildByPath(self.Panel_ElementTrail, "Label_element_trail_desc")
    
    self.Label_element_trail_desc:setTextById(elementTrailCfg.debuffDescribe)
    self.Label_element_trail_buff_desc = TFDirector:getChildByPath(self.Panel_ElementTrail, "Label_element_trail_buff_desc")
    self.Label_element_trail_buff_desc:setTextById(elementTrailCfg.buffDescribe)
    local ScrollView_element_trail_buff = TFDirector:getChildByPath(self.Panel_ElementTrail, "ScrollView_element_trail_buff"):hide()
    -- local listview__element_trail_buff = UIListView:create(ScrollView_element_trail_buff)

    --关卡奖励
    local ScrollView_award = TFDirector:getChildByPath(self.Panel_ElementTrail, "ScrollView_award")
    local listview_reward = UIListView:create(ScrollView_award)

    local rewards = self.levelCfg_.firstDropShow or {}
    -- dump(rewards)
    -- Box("11")
    for _, goodsId in pairs(rewards) do
       ...
       local Panel_dropGoodsItem = PrefabDataMgr:getPrefab("Panel_goodsItem"):clone()
        Panel_dropGoodsItem:Scale(0.6)
        print("goodsId:".. goodsId)
        PrefabDataMgr:setInfo(Panel_dropGoodsItem, goodsId, 1)
        listview_reward:pushBackCustomItem(Panel_dropGoodsItem)
    end
...
function FubenSquadView:refreshView()
    ...
    elseif self.fubenType_ == EC_FBType.ACTIVITY then
        ...
        elseif self.chapterCid_ == EC_ActivityFubenType.SNOW_FESVITAL then
            ...
            self.Panel_formation:show()
        elseif self.chapterCid_ == EC_ActivityFubenType.ELEMENT_TRIAL then
            self.Panel_formation:show()
            self:showElementTrail()
        end
    elseif self.fubenType_ == EC_FBType.HOLIDAY or self.fubenType_ == EC_FBType.HOLIDAY2 then
        if self.chapterCid_ == EC_ActivityFubenType.HALLOWEEN then
            self.Panel_formation:show()
...
function FubenSquadView:onFightingClick()
    ...
    --元素试炼 疲劳度检查
    if self.chapterCid_ == EC_ActivityFubenType.ELEMENT_TRIAL then 
        for i, v in ipairs(self.formationData_) do
            local value = ActivityDataMgr:getElementTrailHeroFatigue(v.id)
            if value >= 100 then 
                  Utils:showTips("队伍中有成员疲劳度过高，无法进行战斗")
                return 
            end
        end
    end
...
function FubenSquadView:updateFormation()
    for i, v in ipairs(self.Panel_member) do
        ...
        if isBattle then
            ...
            local model = Utils:createHeroModel_( v.Panel_model ,heroData.skinCid ,"battleSize")
            model:update(0.1)
            model:stop()
            ...
            elseif self.chapterCid_ == EC_ActivityFubenType.ELEMENT_TRIAL then
                --疲劳度展示
                local tiredValue = ActivityDataMgr:getElementTrailHeroFatigue(heroData.id)
                -- local percent    = math.min(1,tiredValue / 100)
                -- print("-----------00000000000000000000000000000000000000000000000000")
                -- print(percent)
                -- self:updateElementTrail() 
                v.Panel_element_trail:show()
                v.Label_tired_element_trail:show()
                v.Label_tired_element_trail:setText(tiredValue.."%")
                v.LoadingBar_tired_element_trail:setPercent(tiredValue)
                local isUp = self:checkElementTrainStaminaHero(heroData.id)
                v.Image_up_element_trail:setVisible(isUp)
                --疲劳度过大不能参站
                v.Image_disable_type:setVisible(tiredValue >= 100)
                v.Label_disable_type:setTextById(100000039) 
            end
			--更新克制icon
...
function FubenSquadView:updateAssistantData()
    self.assistantData_ = {}
    local assistHeros = FubenDataMgr:getAssistHeros()
    for i, assistHero in ipairs(assistHeros) do
