...
function FubenReadyView:initUI(ui)
    ...
    --扫荡按钮
    self.Button_sweep = TFDirector:getChildByPath(self.Panel_fighting, "Button_sweep") --:hide()
...
--关卡是否可以扫荡
function FubenReadyView:canSweep(dungeonType)
    local data = TabDataMgr:getData("DiscreteData",21009).data
    return table.find(data.annDunGroupTypeList, dungeonType ) ~= -1 
end    
...
-- Box("xx:"..tostring(self.levelCfg_.superType))
    ...
    --主线战斗关卡 非飞行模式，才能扫荡
    self.Button_sweep:setVisible(self:canSweep(self.levelGroupCfg_.dungeonType) and self.levelCfg_.fightingMode == 1)
    ...
    local nodes = {self.Button_ready}
    if self.Button_sweep:isVisible() then 
        table.insert(nodes ,self.Button_sweep)
    end
    if self.Button_select:isVisible() then 
        table.insert(nodes ,self.Button_select)
    end
    if self.Button_cost:isVisible() then 
        table.insert(nodes ,self.Button_cost)
    end

    local nodeR = nil
    for i,v in ipairs(nodes) do
       if nodeR then 
            v:Pos(nodeR:Pos() - ccp((v:getSize().width + nodeR:getSize().width)/2 + 4 ,0))
       end 
       nodeR = v 
    end
    ...
    -- if self.Button_select:isVisible() then  
    --     if self.Button_sweep:isVisible() then 
    --         self.Button_select:Pos(self.Button_sweep:Pos() + ccp( -65 ,0))

    --     else
    --         self.Button_select:Pos(self.Button_sweep:Pos())
    --     end
    --     self.Button_cost:Pos(self.Button_select:Pos() + ccp( -90 ,0))
    -- else

    --     if self.Button_sweep:isVisible() then 
    --         self.Button_cost:Pos(self.Button_sweep:Pos() + ccp( -90 ,0))
    --     else
    --         self.Button_cost:Pos(self.Button_sweep:Pos() + ccp( -25 ,0))
    --     end
    -- end
...
function FubenReadyView:registerEvents()
    EventMgr:addEventListener(self, EV_FUBEN_LEVELINFOUPDATE, handler(self.onLevelInfoUpdate, self))
    ...
    self.Button_sweep:onClick(function()
        if self:canSweep(self.levelGroupCfg_.dungeonType) then
            --检查扫荡条件 
            if not FubenDataMgr:is3StarPassLevel(self.levelCid_) then 
                Utils:showTips(1800133)
                return 
            end
            ...
            local isDuelMod = self.CheckBox_earnings:getSelectedState()
            --正常扫荡
            FubenDataMgr:SEND_DUNGEON_REQ_ANNIHILATE( self.levelCid_, challengeCount, isDuelMod)
        end
            --Utils:openView("common.ConfirmSweepView",self.levelCid_)
    end)
...
function FubenReadyView:onLevelInfoUpdate()
    self:refreshView()
end
