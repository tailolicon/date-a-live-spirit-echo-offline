...
local t = {
    --分辨率
    {{960, 640},{1024, 768},{1136, 640},{1280, 720},{1386, 640},{1600,720},{2048 , 1536 },{2340,1080},{2400,1080},{2436 , 1125},{3840,1644},{2700,1200},{2800,1200}},
    --货币
...
function GMView:initData()
    ...
    if not _baoshi then 
        _baoshi = true
        local stones = TabDataMgr:getData("Stone")
        local stoneIds = {}
        for k,v in pairs(stones) do
            table.insert(stoneIds,k)
        end
        table.sort(stoneIds)
        table.insert(t,stoneIds)
        table.insert(headName,"宝石")
    end
...
function GMView:ctor(...)
    ...
    self:initData(...)
    self:init("lua.uiconfig.secondary.uiconfig_zn.test.gmView")
...
function GMView:initUI(ui)
    ...
    self.Button_showCid = TFDirector:getChildByPath(self.Panel_quick, "Button_showCid")
    ...
    if cell.item then
        if name == "Head" then
            ...
            if selectnum == 1 then
                ...
                input:show();
                if Concent[idx] > 1301000 then 
                    nameLab:setTextById(TabDataMgr:getData("Stone",Concent[idx]).nameTextId);
                else
...
function GMView:registerEvents()
    ...
    if self.Button_showCid then 
        self.Button_showCid:onClick(function()
            if not GlobalHideItemCid then 
                GlobalHideItemCid = true
            else
                GlobalHideItemCid = false
            end
        end)
    end
end
