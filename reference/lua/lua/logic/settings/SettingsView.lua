...
local function __setPos(nodes ,start)
    start = start or -5
    for i,node in ipairs(nodes) do
        node:setPositionY((i-1)*-95  + start )
    end
end
...
function SettingsView:initData(roleCid)
    ...
    self.tmsettingData.dating.optionSkip = SettingDataMgr:getOptionSkip()
    ...
    self.tmsettingData.other.summonAnimation = SettingDataMgr:getSummonAnimation()
...
function SettingsView:initPanelbtns()
    ...
    if CC_TARGET_PLATFORM == CC_PLATFORM_IOS  then
        showBtnList   = {Button_dating, Button_sound, Button_battle, Button_collect, Button_MainPage, Button_other, Button_language,Button_darkModel, Button_touch }
    ...
    elseif CC_TARGET_PLATFORM == CC_PLATFORM_WIN32  then
        if SINGLE_LANG_VERSION then 
            showBtnList   = {Button_dating, Button_sound, Button_battle, Button_collect, Button_MainPage, Button_other, Button_darkModel}
            self.btn_list = {dating = Button_dating, sound = Button_sound, battle = Button_battle, collect = Button_collect, mainPage = Button_MainPage, other = Button_other ,darkModel = Button_darkModel}
        else
    ...
    local function saveCurSettingData()
        ...
        SettingDataMgr:setOptionSkip(self.tmsettingData.dating.optionSkip)
            ...
            SettingDataMgr:setSummonAnimation(self.tmsettingData.other.summonAnimation)
...
function SettingsView:setOptionSkip(value)
    value = self.tmsettingData.dating.optionSkip + 1
    if value > 2 then
        value = 1
    end
    self.tmsettingData.dating.optionSkip = value
    self:updateOptionSkip()
end
...
function SettingsView:updateOptionSkip()
    local Panel_option = TFDirector:getChildByPath(self.Panel_dating, "Panel_option")
    local value = self.tmsettingData.dating.optionSkip

    for i=1,1 do
        local btn = TFDirector:getChildByPath(Panel_option, "Button_common_"..i)
        local movepos = me.p(-33,0)
        if value ~= 1 then
            movepos = me.p(33,0)
        end
...
function SettingsView:setSummonAnimation(redPack)
    local value = self.tmsettingData.other.summonAnimation + 1
    if value > 2 then
        value = 1
    end
    self.tmsettingData.other.summonAnimation = value
    self:updateSummonAnimation()
end

function SettingsView:updateSummonAnimation()
    local Panel_summon = TFDirector:getChildByPath(self.Panel_other, "Panel_summon")
    local value = self.tmsettingData.other.summonAnimation

    for i=1,1 do
        local btn = TFDirector:getChildByPath(Panel_summon, "Button_common_"..i)
        local movepos = me.p(-33,0)
        if value == 1 then
            movepos = me.p(33,0)
        end
...
function SettingsView:initPanelSound()
    ...
    local Image_sound_effect = TFDirector:getChildByPath(self.Panel_sound, "Panel_vol_effect")
    local Image_sound_voice = TFDirector:getChildByPath(self.Panel_sound, "Panel_vol_speak")
    __setPos({Image_sound_main,Image_sound_bgm,Image_sound_effect,Image_sound_voice})
    for i = 1, 4 do 
...
function SettingsView:initPanelDating()
    ...
    local Panel_cg = TFDirector:getChildByPath(self.Panel_dating, "Panel_cg")
    local Panel_option = TFDirector:getChildByPath(self.Panel_dating, "Panel_option")
    __setPos({Image_speed_text,Panel_cg,Panel_option,Image_speed_skip,Image_speed_auto})
    ...
    for i=1,1 do
        local btn = TFDirector:getChildByPath(Panel_option, "Button_common_"..i)
        btn:onClick(function()
            self:setOptionSkip(i)
        end)
    end

    TFDirector:getChildByPath(Panel_option, "Label_close"):setSkewX(15)
    TFDirector:getChildByPath(Panel_option, "Label_open"):setSkewX(15)
...
function SettingsView:initPanelCollect()
    ...
    self.collect_listView:setItemsMargin(5)
...
function SettingsView:initPanelBattle()
    ...
    __setPos({Panel_common,panel_battle_fps ,panel_battle_roke ,Panel_awake_effect ,Panel_damage ,Image_battle_blur},480)
end
...
function SettingsView:initPanelLanguage()
    ...
    __setPos({self.Panel_language_en})
...
function SettingsView:initPanelOther()
    ...
    local Panel_dating = TFDirector:getChildByPath(self.Panel_other, "Panel_dating")
    local Panel_fight = TFDirector:getChildByPath(self.Panel_other, "Panel_fight")
    __setPos({Panel_summon,Panel_mainLayer,Panel_dating,Panel_fight})

    --TODO 屏蔽红包相关开关，使用Panel_mainLayer 作为召唤动画的开关项 ，需要指定Label_title多语言的文本ID 
    
    Panel_mainLayer:hide()
    Panel_dating:hide()
    Panel_fight:hide()
    for i=1,1 do
        local btn = TFDirector:getChildByPath(Panel_summon, "Button_common_"..i)
        btn:onClick(function()
            self:setSummonAnimation(i)
        end)
    end
    TFDirector:getChildByPath(Panel_summon, "Label_close"):setSkewX(15)
    TFDirector:getChildByPath(Panel_summon, "Label_open"):setSkewX(15)
    --TFDirector:getChildByPath(Panel_mainLayer, "Label_title"):setText("神奇的大象")
...
function SettingsView:initPanelDarkModel( )
    local Panel_darkModel = TFDirector:getChildByPath(self.Panel_darkModel , "Panel_en_dark")
    __setPos({Panel_darkModel})
    for i=1,3 do
