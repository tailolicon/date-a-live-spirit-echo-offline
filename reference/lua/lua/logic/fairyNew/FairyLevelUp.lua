...
function FairyLevelUp:registerEvents()
 ...
 --    	end)
	...
	self._ui.Button_min:onClick(function()
		self:resetUI()
	end)

	self._ui.Button_max:onClick(function()
		if not self.items[self.curidx] then
			return
		end
		if not HeroDataMgr:isCanLevelUp(self.heroid) then
			return
		end
		local needExp = HeroDataMgr:getToMaxLevelNeedExp(self.heroid)
		local itemExp = TabDataMgr:getData("Item", self.items[self.curidx].itemId).useProfit.fix.items[1].num
		self.selectNum = math.min(math.ceil(needExp / itemExp), self.items[self.curidx].num)
		self:updateUI(0)
	end)
