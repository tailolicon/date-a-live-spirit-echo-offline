...
local function __setAttrInfo(node,attrId)
    local linePannel = node:getChildByName("Line_4")
    local icon       = linePannel:getChildByName("TypeBg")
    local discTxt    = linePannel:getChildByName("LabDisc")
    local attrCfg    = HeroDataMgr:getAttributeConfig(attrId)
    icon:setTexture(attrCfg.icon)
    discTxt:setTextById(attrCfg.name)
end
...
function BeforeEvloutionShowView:initData()
    ...
    self.quality     = heroConfig.quality
    ...
    if heroConfig.quality > 6 then 
        self.dataOld[4] = math.floor(TabDataMgr:getData("HeroProgress",progressId).baseAttr[EC_Attr.HIT] / 100)
        self.dataNew[4] = math.floor(TabDataMgr:getData("HeroProgress",progressIdNew).baseAttr[EC_Attr.HIT] / 100)
    end
end
...
function BeforeEvloutionShowView:setRoleAttributeTxt(node, data)
    ...
    if self.quality > 6 then 
        __setAttrInfo(node,EC_Attr.HIT)
    end
end
