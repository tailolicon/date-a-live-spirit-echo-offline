...
function StrengthenResult:initUI(ui)
	...
	local hp_gr_sk          = TFDirector:getChildByPath(ui,"hp_gr_sk");
	...
	if self._type == 2 then 
		if self.oldHero.quality > 6  then
			local oldProgressId   = self.oldHero.attribute * 100  + self.oldHero.quality 
			old_gr_sk:setString(math.floor(TabDataMgr:getData("HeroProgress",oldProgressId).baseAttr[EC_Attr.HIT] / 100))
			cur_gr_sk:setString(math.floor(TabDataMgr:getData("HeroProgress",progressId).baseAttr[EC_Attr.HIT] / 100));
			hp_gr_sk:setTextById(1329114)
		end
	end
