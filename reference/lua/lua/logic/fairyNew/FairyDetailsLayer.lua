...
function FairyDetailsLayer:ctor(data)
    ...
    table.insert(self.tabBtnInfo , {name = TextDataMgr:getText(1706461), icon = "ui/fairy/new_ui/tab_9.png"})
...
function FairyDetailsLayer:initUI(ui)
	...
	Utils:setFontName(self.Label_hero_name)
	...
	self.Button_angel_strengthen = TFDirector:getChildByPath(self.Panel_angel,"Button_angel_strengthen")
	self.Label_angel_strengthen = TFDirector:getChildByPath(self.Button_angel_strengthen,"Label_angel_strengthen")
	...
	self.Panel_pet		= TFDirector:getChildByPath(ui, "Panel_pet"):hide()		--宠物UI
	self:initPetUI()
	self.curPanel			= nil
	...
	table.insert(self.tabPanels,{pl = self.Panel_pet,uiType = EC_FairyDetailUIType.Pet})
	if GlobalFuncDataMgr:isOpen(8) then
	...
	local skyLadderShowId = {[3] = 1,[4] = 1,[6] = 1}
...
--初始化宠物UI
function FairyDetailsLayer:initPetUI()
	local Panel   = TFDirector:getChildByPath(self.Panel_pet, "Panel")
	self.Panel_pet_base		= TFDirector:getChildByPath(Panel, "Panel_pet_base")
	self.Panel_pet_none		= TFDirector:getChildByPath(Panel, "Panel_pet_none"):hide()		
	self.Panel_pet_normal	= TFDirector:getChildByPath(Panel, "Panel_pet_normal"):show()	

	self.Button_change_pet = TFDirector:getChildByPath(self.Panel_pet_base, "Button_change_pet")
	self.Button_upgrade_pet = TFDirector:getChildByPath(self.Panel_pet_normal, "Button_upgrade_pet")
	self.Label_change = TFDirector:getChildByPath(self.Button_change_pet, "Label_change")
	self.Label_upgrade = TFDirector:getChildByPath(self.Button_upgrade_pet, "Label_upgrade")
	self.Button_change_pet:setVisible(not self.isfriend )
	self.Button_upgrade_pet:setVisible(not self.isfriend)

	self.Image_redTip_change_pet = TFDirector:getChildByPath(self.Button_change_pet, "Image_redTip"):hide()
	self.Image_redTip_upgrade_pet= TFDirector:getChildByPath(self.Button_upgrade_pet, "Image_redTip"):hide()

	self.Label_pet_name = TFDirector:getChildByPath(self.Panel_pet_normal, "Label_pet_name")
	self.Image_pet =  TFDirector:getChildByPath(self.Panel_pet_normal, "Image_pet") --宠物动画节点
	self.nodePetStars   = {}
	local Panel_star =  TFDirector:getChildByPath(self.Panel_pet_base, "Panel_star")
	for i=1,6 do
	...
	self.Label_pet_skill_desc = TFDirector:getChildByPath(self.Panel_pet_normal, "Label_pet_skill_desc")
	local Panel_pet_level =TFDirector:getChildByPath(self.Panel_pet_normal, "Panel_pet_level")
	...
	for i=1,3 do --属性以此 攻击、防御、血量 
		...
		local Panel_pet_attr_item      = TFDirector:getChildByPath(self.Panel_pet_normal, "Panel_pet_attr_item"..i)
	...
	--多语言适配
	local Label_attr_title_name		   = TFDirector:getChildByPath(self.Panel_pet_base, "Label_attr_title_name")
	local Label_skill_title_name       = TFDirector:getChildByPath(self.Panel_pet_base, "Label_skill_title_name")
	local Label_no_pet1		           = TFDirector:getChildByPath(self.Panel_pet_none, "Label_no_pet1")
	local Label_no_pet2                = TFDirector:getChildByPath(self.Panel_pet_none, "Label_no_pet2")

	Label_attr_title_name:setTextById(290000098)
	Label_skill_title_name:setTextById(290000099)
	Label_no_pet1:setTextById(290000100)
	Label_no_pet2:setTextById(290000100)
	self.Label_change:setTextById(1100007)
	self.Label_upgrade:setTextById(290000101)
end

--显示宠物星
function FairyDetailsLayer:refeshPetStar(star,maxStar)
...
function FairyDetailsLayer:updatePetLayer()
	local petData  = HeroDataMgr:getPet(self.showHeroId) 
	self.Panel_pet_normal:setVisible(petData ~= nil)
	self.Panel_pet_none:setVisible(not petData)
	if petData then 
		local petId  = petData.cid
		local petCfg  = PetDataMgr:getPetCfg(petId)   

		local des2    = PetDataMgr:getPetSkillDes(petId,petData.star) 
		--名称
		self.Label_pet_name:setTextById(petCfg.nameTextId)
		--登录
		self.Label_pet_skill_desc:setTextById(des2)
		--等级
		self.Label_pet_level:setText(tostring(petData.level))

		self.Label_pet_level_max:setText("")
		--属性
 		local attrs = PetDataMgr:getAttributes(petId, petData.star, petData.level)
		...
		self:refeshPetStar(petData.star,petCfg.endStar)
		--宠物spine 创建
		...
		self:refeshPetStar(0,5)
	end
end

--宠物装备
function FairyDetailsLayer:onPetChange()
	self:updatePetLayer()
end
...
function FairyDetailsLayer:registerEvents()
	...
	self.Button_angel_strengthen:onClick(function()
		-- self.angelBreakLevel = HeroDataMgr:getAngelBreakLevel(self.showHeroId,self.isfriend)
		--天使强化
		Utils:openView("fairyNew.AngleStrengthenMain",{heroId = self.showHeroId})
	end)
	...
	self.Button_change_pet:onClick(function()
		local datas = PetDataMgr:getPetDatas()
		if table.count(datas) > 0 then 
        	Utils:openView("fairyNew.PetChangelView", self.showHeroId)
		else
			Utils:showTips(290000112)
		end
		-- Utils:openView("fairyNew.PetStarUpSuccess", 0)
	end)

	--宠物升级
	self.Button_upgrade_pet:onClick(function()
		local petData = HeroDataMgr:getPet(self.showHeroId)
		local upgradeType = PetDataMgr:getUpgradeType(petData.id)
		if upgradeType ~= 0 then
			Utils:openView("fairyNew.PetStrengthenView", petData.id)
		else
			Utils:showTips("宠物已满级")
		end
	end)
end
...
function FairyDetailsLayer:onBottomBtnTouch(idx)
	...
	elseif idx == 7 and isUseIdx then
	...
	local showAngelFlag = 0 
	if self.curPanel == self.Panel_angel or self.curPanel == self.Panel_baoshi then
		showAngelFlag = 1
	elseif  self.curPanel == self.Panel_pet then
		showAngelFlag = 2
	end
	 
	if self.angelModel then
		self.angelModel:setVisible(showAngelFlag == 1)
	end
	if self.anim_hero then
		self.anim_hero:setVisible(showAngelFlag < 1)
	end
	self.Panel_name_info:setVisible(showAngelFlag < 1)
	--self.Panel_suit:setVisible(self.curPanel ~= self.Panel_angel)
	self.Button_eval:setVisible(showAngelFlag < 1 and not self.isfriend)
	--self.Button_eval:setVisible(false)
	self.Button_camera:setVisible(showAngelFlag < 1)
	--self.Button_share:setVisible(self.curPanel ~= self.Panel_angel)
	self.Image_hero_power_bg:setVisible(showAngelFlag < 1)

	self.Image_background:setTexture(showAngelFlag < 1 and "ui/common/bg2.png" or "ui/fairy_skill/BG.png")
	self.Image_suit_bg:setVisible(showAngelFlag < 1)
	self.Label_suit_name:setVisible(showAngelFlag < 1)
	if self.skyladder then
...
function FairyDetailsLayer:updateAngelLayer()
    ...
    self.Button_angel_strengthen:hide()
    ...
    if MainPlayer:getPlayerLv() >= limitLevel then
        if GlobalFuncDataMgr:isOpen(19) then
        	if not self.isfriend and HeroDataMgr:checkAngelBreakUpEnable(self.showHeroId) then
		    	...
		    	if HeroDataMgr:isOpenAngelStrengthen(self.showHeroId) then 
		    		self.Button_angel_strengthen:show()
		    		self.Label_angel_strengthen:setText(HeroDataMgr:getHeroWeaponName(self.showHeroId)..strTag..TextDataMgr:getText(10221042))

		    		self.Button_angel_strengthen:setContentSize(CCSize(self.Label_angel_strengthen:getContentSize().width + 10 , 56))
		    		self.Button_angel_break:setPositionX(-266)
		    	else
				   	self.Button_angel_break:setPositionX(-190)
				end
    ...
    if self.anim_hero then
    	local showAngelFlag = self.curPanel == self.Panel_angel or self.curPanel == self.Panel_baoshi or self.curPanel == self.Panel_pet
