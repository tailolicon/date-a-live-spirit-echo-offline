...
local _RarityTexts = {"D","C","B","A","S","SS","SSS"} 
	...
	self.btn_compose:onClick(function ( ... )
		if self.curStage ~= 4 then
			...
			if isEnough then
				...
				--1100001 ,14300200
				local textNameId = self.showType  == self.types[1]  and 1100001 or  14300200

				local rarityText = _RarityTexts[self.curRarity] or "  "

				local heroName = TextDataMgr:getText(18000368)
				if self.curHero and self.curHero > 0 then
					local heroCfg = TabDataMgr:getData("Hero",self.curHero)
					heroName = TextDataMgr:getText(heroCfg.name)
				end 
				local skillName = TextDataMgr:getText(18000368)
				if self.curHero and self.curHero > 0  and self.curSkillType and self.curSkillType > 0 then 
					local stoneCfg  =  self.stoneData[self.curHero][self.showType][self.curRarity][self.curSkillType]
					--local stoneCfg  =  TabDataMgr:getData("Stone",self.curSkillType)
					skillName = stoneCfg.skillName
				end
				local targetNameId = self.showType  == self.types[1]  and 1100001 or  14300200
				local targetName  = TextDataMgr:getText(targetNameId)
                local args = {
                    tittle = 2107025,
                    -- cancleId = 1329129,
                    -- showCancle = true,
                    reType = "composeShowtip",
                    content = TextDataMgr:getText(18000367,heroName,skillName,rarityText,targetName) ,   -- string.format("是否要铸造[%s]角色的[%s]技能%s级%s",heroName,skillName,rarityText,targetName) , --TextDataMgr:getText(14210307),
