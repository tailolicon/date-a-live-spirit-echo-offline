...
function EquipmentStrengthen:ctor(data)
    ...
    -- self.allItems = {};
    self.selectTab = {};
    self.selectDatas = {};  --TODO 多选情况下记录多远的数据
    self.loadedIndex_ = 1
...
function EquipmentStrengthen:initUI(ui)
    ...
    self:initTableView(ScrollView_Right)
    --装备信息界面
    ...
    --解决经验质点数量选择无法点击的问题
    self.Panel_batch:setZOrder(1) 
-- self.Panel_batch:retain()
-- self.Panel_batch:removeFromParent()
-- Panel_right:addChild(self.Panel_batch)
-- self.Panel_batch:release()

    self:chooseEquipType(Enum_EquipType.Exp)
end

function EquipmentStrengthen:initTableView(scrollView)
    ...
    self.tableView:onCellSize(function()
        ...
        local size = self.rightItem:getContentSize() 
    ...
    self.tableView:onCellAtIndex(function(tableView, idx)
        ...
        if nil == cell then
            ...
            local itemSize  = self.rightItem:getContentSize()
            cell = TFTableViewCell:create()
            cell.items = {} 

            local itemRoot = self.rightItem:clone()
            itemRoot:show()
            itemRoot:setPosition(ccp(0,0))
            cell:addChild(itemRoot)
            cell.items= {}
            for i=1,4 do
                local item = TFDirector:getChildByPath(itemRoot,"item"..i)

                item.Image_level_bg = TFDirector:getChildByPath(item,"Image_level_bg")
                item.Label_lv_title = TFDirector:getChildByPath(item,"Label_lv_title")
                item.LvLabel = TFDirector:getChildByPath(item,"Label_lv");
                item.Image_starList ={}

                --星级
                for i=1,5 do
                    item.Image_starList[i] =TFDirector:getChildByPath(item,"Image_star"..i)
                end
                item.Panel_star = TFDirector:getChildByPath(item,"Panel_star")

                item.Image_stage1 = TFDirector:getChildByPath(item.Panel_star,"Image_stage1")
                item.Image_stage2 = TFDirector:getChildByPath(item.Panel_star,"Image_stage2")
                item.Image_stage3 = TFDirector:getChildByPath(item.Panel_star,"Image_stage3")
                item.Image_back    = TFDirector:getChildByPath(item,"Image_back");

                --是否使用中

                item.useIcon       = TFDirector:getChildByPath(item,"Image_use");

                item.icon          = TFDirector:getChildByPath(item,"Image_equip");

                item.selectImg     = TFDirector:getChildByPath(item,"Image_select");

                item.typeIcon      = TFDirector:getChildByPath(item,"Image_type");
               
                item.backImg       = TFDirector:getChildByPath(item,"Image_back");

                item.Label_num = TFDirector:getChildByPath(item,"Label_num"):hide()
                item.backImg:setTouchEnabled(true)
                -- backImg:onClick(function()
                --         self:onTouchOneEquip(item);
                --     end)
                --item.Image_tuijian = TFDirector:getChildByPath(item,"Image_tuijian"):hide()
                cell.items[i]  =item   
            end


        end
        self:updateCellItem(cell ,idx +1)
        return cell
    end)
...
function EquipmentStrengthen:updateCellItem(cell,idx)
    for i=1,4 do
        local item  = cell.items[i]
        local datas = self.showList[idx]
        if i<= #datas then 
            local data = datas[i]  
            item.data = data          
            item:show()
            self:updateOneEquipment(data,item) 
        else
            item:hide()
        end
    end
...
function EquipmentStrengthen:updateOneEquipment(data,item)
    ...
    item.Image_level_bg:setTexture(EC_ItemLevelIcon[quality])
    item.Label_lv_title:setString("Lv.")
    item.LvLabel:setString(EquipmentDataMgr:getEquipLv(id));


    item.Label_num:hide()
    local num = EquipmentDataMgr:getEquipNum(id)
    if num then
        item.Label_num:setVisible(true)
        item.Label_num:setText(num)
    end

    --星级
    for i=1,5 do
        item.Image_starList[i]:setVisible(i <= starLv)  
    end
    ...
    if starLevel >= 1 and starLevel < 2 then
        item.Panel_star:setScale(0.88)
    elseif starLevel >= 2 then
        item.Panel_star:setScale(0.8)
    else
        item.Panel_star:setScale(1.0)
    end
    ...
    item.Image_stage1:setVisible(starLevel >= 1 and starLevel <= 2)
    item.Image_stage2:setVisible(starLevel == 2)
    item.Image_stage3:setVisible(starLevel == 3)

    -- local Image_back    = TFDirector:getChildByPath(item,"Image_back");
    item.Image_back:setTexture(EC_ItemIcon[quality]);
    --Image_back:setSize(CCSizeMake(105,105));

    --是否使用中
    
    --print("idddddddddddddddddddddddddddddddddddddddddd"..tostring(id))
    local isuse         = EquipmentDataMgr:isUesing(id);
    -- local useIcon       = TFDirector:getChildByPath(item,"Image_use");
    item.useIcon:setVisible(isuse);
    ...
    item.icon:Size(CCSizeMake(100,100));
    ...
    if self.chooseType == Enum_EquipType.Exp then
        if self.selectExpItem and self.selectExpItem.data.id == data.id then
            item.selectImg:setVisible(true)
        else
            item.selectImg:setVisible(false)
        end
    else --多选
        item.selectImg:setVisible( self.selectDatas[data] ~=nil)
    end

    --local typeIcon      = TFDirector:getChildByPath(item,"Image_type");
    local subType       = EquipmentDataMgr:getEquipSubType(id);
    item.typeIcon:setTexture(EC_EquipSubTypeIcon2[subType]);
    item.typeIcon:Size(CCSizeMake(24,24));

    --local backImg       = TFDirector:getChildByPath(item,"Image_back");
    item.backImg:setTouchEnabled(true);
    item.backImg:onClick(function()
        if self.chooseType == Enum_EquipType.Exp then
...
function EquipmentStrengthen:onTouchOneEquip(item)
    if item.selectImg:isVisible() then
        ...
        self.selectDatas[item.data] = nil
    else
        ...
        self.selectDatas[item.data] = true
    end
...
function EquipmentStrengthen:onTouchFastBtn()
    ...
    for i, v in ipairs(self.showList) do
        for _, data in ipairs(v) do
            if EquipmentDataMgr:getEquipStarLv(data.id) < 4 then
                isHave = true
                local level ,curPercent;
                self.isMaxLevel,level,curPercent = EquipmentDataMgr:calcLevelUp(self.showId,self.selectExp);
        
                if not self.selectDatas[data] and not self.isMaxLevel then
                    if data.merge then
                        local totalExp = 0
                        for k,v in ipairs(data.merge) do
                            totalExp = totalExp + EquipmentDataMgr:getEquipTotalExp(v);
                            table.insert(self.selectTab,v);
                        end
                        self.selectExp = self.selectExp + totalExp
                    else
                        self.selectExp = self.selectExp + EquipmentDataMgr:getEquipTotalExp(data.id);
                        table.insert(self.selectTab,data.id);
                    end
                    self.selectDatas[data] = true
                end
            end
        end
    end
