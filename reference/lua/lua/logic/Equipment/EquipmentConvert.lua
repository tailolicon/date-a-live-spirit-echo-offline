...
function EquipConvert:initTableView(scrollView)
...
function EquipConvert:updateCellItem(cell,idx)
    for i=1,4 do
        local item = cell.items[i]
        local id   = self.curList[idx][i]  
        item.id = id          
...
function EquipConvert:loadEquipMent()
    self.Label_nothing:setVisible(#self.curList < 1)
