...
function EquipmentSelect:initTableView(scrollView)
...
function EquipmentSelect:updateCellItem(cell,idx)
    for i=1,4 do
        local item = cell.items[i]
        local id   = self.showList[idx][i]
        item.id = id
        self:updateOneEquipment(id,item)
    end
end
...
function EquipmentSelect:updateOneEquipment(id,item)
    ...
    -- if self.recommendCids[cid] then
        item.Image_tuijian:setVisible(self.recommendCids[cid]~=nil)
    -- end
end
