...
function BFEffect:getBuffbenefitType()
    return self.data.benefitType
end
--是否为减少属性类效果
...
function BFEffect:takeEffect(addTimes)
    ...
    -- if objectType == eBFObjectType.OT_HERO then --作用于目标
        ...
        elseif effectType == eBFEffectType.ET_REMOVE_TARBUFFER then
            --移除目标buffer

            print("移除目标Buff------------" .. target:getName())
            local removeType = self.data.removeType
            local removeBuffer = self.data.removeBuffer
            local removeBufferEffect = self.data.removeBufferEffect
            local removeTypeNumber = self.data.removeTypeNumber

            if removeType and removeTypeNumber and removeType ~= 0 and removeTypeNumber ~= 0 then
                local effectMap = target:getBufferEffectMap()
                local effectRemove = {}
                for i, bfEffect in pairs(effectMap) do
                    if bfEffect:getBuffbenefitType() == removeType then
                        table.insert(effectRemove, bfEffect)
                    end
                end
                if #effectRemove <= removeTypeNumber then
                    for i, v in pairs(effectRemove) do
                        v:removeEffect()
                        v:removeSelf()
                    end
                else
                    ...
                    local tabs = {}
                    local cacheTabs = nil
                    for i = 1, removeTypeNumber do
                        local rangIndex = RandomGenerator.random(1, #effectRemove)
                        table.insert(tabs, effectRemove[rangIndex])
                        table.remove(effectRemove, rangIndex)
                        cacheTabs = {}
                        for p, t in pairs(effectRemove) do
                            table.insert(cacheTabs, t)
                        end
                        effectRemove = cacheTabs
                    end
                    for c, b in pairs(tabs) do
                        b:removeEffect()
                        b:removeSelf()
                    end
                end

            elseif removeBuffer and #removeBuffer > 0 then
                for i, v in pairs(removeBuffer) do
                    local buffer = target:getBuffer(v)
                    if buffer then
                        target:removeBuffer(buffer)
                    end
                end
            elseif removeBufferEffect and #removeBufferEffect > 0 then
                for i, v in pairs(removeBufferEffect) do
                    local effect = target:getBFEffect(v)
                    if effect then
                        effect:removeEffect()
                        effect:removeSelf()
                    end
