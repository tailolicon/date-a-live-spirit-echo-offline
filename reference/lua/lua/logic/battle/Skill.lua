...
function Skill:isManual(flag)
    if battleController.getCaptain() == self.hero then 
...
function Skill:checkNext(force)
    ...
    if self.eventData then
        --dump({self.eventData ,self:isManual()})
        if self:isManualX() then
            -- dump(self.eventData)
            ...
            --if self.hero:canNextAttack() then
                local actionID = RANDOM_EVENT(self.eventData)
                   dump("><><><>>trigget next >>>>>>>>>>>:" ..tostring(actionID))
                if actionID then
                    self:playAction(actionID)
                    self.eventData = nil
                    dump("><><><>>trigget next >>>>>>>>>>>")
...
function Skill:onEventTrigger(source,event,target,param)
    ...
    for k,trigger in ipairs(triggers) do
        if battleController.checkCondSuccess(trigger.cond,{skill = self, event = event, source = source, target = target, param = param }) then
            ...
            elseif trigger.type == 2 then  --改变目标
                if trigger.skillId and trigger.skillId > 0 then
                    local skill  = self.hero:getSkillByCid(trigger.skillId)
                    skill:setVisiable(false)
                    if trigger.activeSkillId and trigger.activeSkillId > 0 then
                        local activeSkill  = self.hero:getSkillByCid(trigger.activeSkillId)
                        activeSkill:setVisiable(true)
                    end
                end
            end
        end
