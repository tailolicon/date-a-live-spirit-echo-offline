...
function BattleResultView:initData()
    ...
    if self.battleType_ == EC_BattleType.COMMON or self.battleType_ == EC_BattleType.ARENA then
...
function BattleResultView:initUI(ui)
    ...
    -- 竞技场UI
    self.Panel_arena = TFDirector:getChildByPath(self.Panel_root, "Panel_arena"):hide()
    local Image_arena_score = TFDirector:getChildByPath(self.Panel_arena, "Image_arena_score")
    self.Label_arena_score = TFDirector:getChildByPath(Image_arena_score, "Label_arena_score")
    local Image_arena_formation  = TFDirector:getChildByPath(self.Panel_arena, "Image_arena_formation")
    self.Panel_arena_team  = TFDirector:getChildByPath(Image_arena_formation, "Panel_arena_team") 
    --TODO竞技场文本替换
    local Label_arena_formation_title = TFDirector:getChildByPath(Image_arena_formation, "Label_arena_formation_title")
    -- Label_arena_formation_title:setTextById(13033)
    local Label_arena_score_title = TFDirector:getChildByPath(Image_arena_score, "Label_arena_score_title")
    Label_arena_score_title:setTextById(13033)
    ...
    self.panelRootClickFunc = function()
            if self.isEnableTouching then
                ...
                if self.touchCount_ == 1 then
                    ...
                    -- self.teamAttackPage:hide()
                    -- self.Panel_reward:show()
                    if self.battleType_ == EC_BattleType.ARENA then
...
function BattleResultView:checkBtnShareEnableShow()
    if HeitaoSdk and CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
        return
    end
    ...
    -- required int32 segment = 1; //段位
    -- required int32 arenaScore = 2; //竞技场分
    -- repeated int32 regionBuffs = 12;//区域buff

--显示竞技场结算
function BattleResultView:showArena()
    self.Panel_arena:show()
    self.Panel_evaluation:hide()
    self.Panel_reward:hide()
    -- local data = ArenaDataMgr:getArenaData() or {}

    local battleScore = ArenaDataMgr:getBattleScore()

    if battleScore >= 0 then 
        self.Label_arena_score:setFontColor(ccc3(0,255,0))
        self.Label_arena_score:setTextById(300602 ,battleScore)
    else
        self.Label_arena_score:setText(tostring(battleScore))
        self.Label_arena_score:setFontColor(ccc3(255,0,0))
    end
    local roundData = ArenaDataMgr:getRoundData()
    local matchData = ArenaDataMgr:getMatchData()
    if matchData then 
        local heros = matchData.selfHeroes
        for i=1,3 do
            local roleItem = TFDirector:getChildByPath(self.Panel_arena_team, "Panel_role"..i) 
            local Image_level_bg   = TFDirector:getChildByPath(roleItem, "Image_level_bg"):show() 
            local Label_level      = TFDirector:getChildByPath(roleItem, "Label_level"):show() 
            ...
            local Image_pinzhi     = TFDirector:getChildByPath(roleItem, "Image_pinzhi"):show() 
            local Label_tip        = TFDirector:getChildByPath(roleItem, "Label_tip")
            local data   = roundData[i]
            local hero   = heros[i]
            Label_level:setTextById(700034,hero.lvl)
            local heroCfg = HeroDataMgr:getHero(data.selfHero)
            Image_playerIcon:setTexture(HeroDataMgr:getIconPathById(heroCfg.id, hero.skinCid))
            Image_pinzhi:setTexture(HeroDataMgr:getQualityPicNotHave(heroCfg.id))
            Label_tip:setText(data.success and "胜" or "败")
            Label_tip:setFontColor(data.success and ccc3(0,255,0) or ccc3(255,0,0))
        end
    end
end
