...
function BattleView:ctor(...)
    ...
    --快速通关脚本
    if DEBUG == 1 and me.platform == "win32" then
        TFDirector:registerKeyDown(114, {nGap = 500}, function() -- 'F3'
            if battleController.bStartTime then
...
--TODO 解决控件 ScaleTo 无效果的问题
    ...
    for i = 1 , 2 do
        ...
        roleNode.label_tip     = TFDirector:getChildByPath(roleNode, "Label_tip")
...
--圆形转盘
    ...
    --自动战斗按钮
    self.checkBox_auto_fight = TFDirector:getChildByPath(self.Panel_top, "CheckBox_auto_fight")
    ---1-4  对应的关卡通过才会解锁
    self.enableAuto = FubenDataMgr:isPassPlotLevel(101104)   --自动战斗是否解锁
    --木桩 和组队不显示自动战斗按钮  竞技场不显示自动按钮
    if self.levelType_ == EC_FBLevelType.PRACTICE
        or self.levelType_ == EC_FBLevelType.ARENA 
        or battleController.isLockStep() then
        self.checkBox_auto_fight:setVisible(false)
        self.enableAuto = false
        battleController.setAutoFight(self.enableAuto)
    else
        self.checkBox_auto_fight:setVisible(true)
        battleController.setAutoFight(self.enableAuto)
        self.checkBox_auto_fight:onClick(function()
            if self.enableAuto then 
                 battleController.setAutoFight(not battleController.getAutoFight())
            else
                Utils:showTips(18000340)
            end
        end)
        self:onAutoFightStateChange()
    end

    self.speedIndex   = 1
    self.timeScale    = 1
    self.Button_speed = TFDirector:getChildByPath(self.Panel_top, "Button_speed")
    self.Label_speed  = TFDirector:getChildByPath(self.Button_speed, "Label_speed")
    if self.levelType_ == EC_FBLevelType.ARENA then 
        self.Label_speed:setText("X".._TimeScales[self.speedIndex])
        self.keyBoard.panel_ctrlpad:hide()
        self.Button_speed:show()
        self.Button_speed:setTouchEnabled(true)
        battleController.setAutoFight(true)  --开启自动战斗
    else
        self.keyBoard.panel_ctrlpad:show()
        self.Button_speed:hide()
        self.Button_speed:setTouchEnabled(false)
    end

    -- Box(  "Button pause is visible: "   .. tostring (self.keyBoard.pause_btn:isVisible())  ..tostring(self.keyBoard.Panel_pause:isVisible()    ))
end

function BattleView:autoChangeSpeed()
    self.speedIndex = self.speedIndex  + 1
    if self.speedIndex  > #_TimeScales then 
        self.speedIndex = 1
    end
    self.timeScale = _TimeScales[self.speedIndex] 
    self.Label_speed:setText("X"..self.timeScale)
end

function BattleView:onAutoFightStateChange()
    if self.enableAuto then 
        local auto = battleController.getAutoFight()
        if auto then 
            self.checkBox_auto_fight:setTextureNormal("ui/battle/btn_zd_02.png")
        else
            self.checkBox_auto_fight:setTextureNormal("ui/battle/btn_zd_01.png")
        end
    else
        self.checkBox_auto_fight:setTextureNormal("ui/battle/btn_zd_00.png")
    end
end
...
function BattleView:showPrewarTip()
    ...
    elseif self.battleType_ == EC_BattleType.ARENA then

        if battleController.getArenaRound()  > 0 then 
            local action = Sequence:create({
                    Show:create(),
                    DelayTime:create(2),
                    Hide:create(),
            ...
            self.Panel_endlessTip:runAction(action)
            self.Label_endlessLevel:setText(string.format("第%s回合" , battleController.getArenaRound()))
        end
        -- self.Label_endlessLevel:setTextById(300832, self.levelCfg_.order)
    end
...
function BattleView:refreshVictoryState()
    ...
    elseif self.battleType_ == EC_BattleType.ARENA then
        self:refreshArenaVictoryState()
    end

end

function BattleView:refreshArenaVictoryState()
    local victoryDecide = battleController.getVictoryDecide()
    local _, hour, min, sec = Utils:getDHMS(victoryDecide.nSecondTime, true)
    self.label_victory_time:setText(string.format("%s:%s", min, sec))
end
...
function BattleView:registerEvents()
    ...
    EventMgr:addEventListener(self, eEvent.EVENT_AUTO_FIGHT_STATE_CHANGE, handler(self.onAutoFightStateChange,self))
    ...
    self.keyBoard.pause_btn:onClick(function()
        if self.pause_countdown_time > 0 then
            Utils:showTips(100124)
            return 
        end

        self.pause_countdown_time = 3
        --组队模式
        if self.battleType_ == EC_BattleType.TEAM_FIGHT then
        ...
        elseif self.battleType_ == EC_BattleType.ENDLESS then
            ...
            self:onPause()
         elseif self.battleType_ == EC_BattleType.ARENA then --竞技场战斗
            -- if not battleController.isClearing then
            --     BattleUtils:openView("battle.BattlePauseView")
            --     self:onPause()
            -- end
            Utils:showTips("本次战斗不支持暂停") 
        end
        -- TODO 测试
...
function BattleView:showPause()
    ...
    elseif self.battleType_ == EC_BattleType.ENDLESS then
        ...
        self:onPause()
    elseif self.battleType_ == EC_BattleType.ARENA then
        -- if not AlertManager:isLayerInQueue("PauseView") then  
        --     BattleUtils:openView("battle.BattlePauseView")
        -- end
        -- self:onPause()
        --Utils:showTips("本次战斗不支持暂停") 
...
function BattleView:onCaptainSelect(item)
    ...
    if self.battleType_ == EC_BattleType.ARENA then  return end --竞技场战斗禁用切换角色
...
function BattleView:gameRun(deltaTime)
    ...
    if self.pause_countdown_time > 0 then 
        self.pause_countdown_time = self.pause_countdown_time - deltaTime
        -- print("self.pause_countdown_time:" ..self.pause_countdown_time)
    end
...
function BattleView:endAniCallback(callback)
    ...
    elseif self.battleType_ == EC_BattleType.ARENA then
        ...
        -- self:commonFightResult() --TODO 是否需要特殊处理结算
    end
end
...
function BattleView:onStaceClear(callback)
    if self.levelCfg_ and self.levelCfg_.isPlayVictorAction then  --是否展示胜利失败动画
        ...
        if self.levelCfg_.dungeonType == EC_FBLevelType.TONG_MONSTER or 
            self.levelCfg_.dungeonType == EC_FBLevelType.ARENA then
...
function BattleView:normalFightResult()
    if self.endMsgFlag_ and self.endAniFlag_ then
        self.endMsgFlag_ = false
        self.endAniFlag_ = false
        if self.levelType_ == EC_FBLevelType.TONG_MONSTER or
            self.levelType_ == EC_FBLevelType.ARENA then
