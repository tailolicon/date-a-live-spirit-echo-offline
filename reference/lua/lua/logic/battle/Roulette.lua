...
function MemberList:resetItem(roleNode ,hero)
    ...
    if BattleDataMgr:getBattleType() == EC_BattleType.ARENA then 
        roleNode.image_dead:setVisible(false)  
        local state = hero:getArenaFightState()
        if state == 1 then 
            roleNode.label_tip:setText("Win")
            roleNode.label_tip:setFontColor(ccc3(0,255,0))
        elseif state == -1 then 
            roleNode.label_tip:setText("Lose")
            roleNode.label_tip:setFontColor(ccc3(255,0,0))
        else
            roleNode.label_tip:setText("") 
        end
    else
        roleNode.image_dead:setVisible(hero:isDead())  
        roleNode.label_tip:setVisible(false)
    end
        ...
        if hero then
            if _hero == nil  or _hero == hero then
                ...
                if BattleDataMgr:getBattleType() == EC_BattleType.ARENA then 
                    ...
                    local state = hero:getArenaFightState()
                    if state == 1 then 
                        roleNode.label_tip:setText("胜")
                        roleNode.label_tip:setFontColor(ccc3(0,255,0))
                    elseif state == -1 then 
                        roleNode.label_tip:setText("败")
                        roleNode.label_tip:setFontColor(ccc3(255,0,0))
                    else
                        roleNode.label_tip:setText("") 
                    end
