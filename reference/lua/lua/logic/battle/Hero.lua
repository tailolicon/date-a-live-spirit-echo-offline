...
local petMgr      = BattleMgr.getPetMgr() --宠物管理器
...
function Hero:ctor(data,team,host)
    ...
    if host then --如果有召唤宿主 保存召唤宿主ID
        ...
        if host:getRoleType() == eRoleType.Hero or host:getRoleType() == eRoleType.Hero_Arena then
    ...
    if BattleDataMgr:getBattleType() == EC_BattleType.ARENA  then
        if  self.data.roleType  == eRoleType.Monster  and self.host then 
            if host:getRoleType() == eRoleType.Hero  then
                self.data.camp = 11
            elseif host:getRoleType() == eRoleType.Hero_Arena then
                self.data.camp = 12
            end
        end
    end
...
function Hero:enableUpdateBossPanel()
    -- if self:getCamp() == 71 then 
    --     return true --血量作为Boss 显示 
    -- end
...
function Hero:changeSkin(formId,force)
    ...
    or roleType == eRoleType.Trap
    or roleType == eRoleType.Hero 
    or roleType == eRoleType.Hero_Arena then
        self:endToAI() 
    ...
    if self.curForm and self.curForm.aiAgent then
        --Box("aaa")
        dump({"角色AI hero: "..self.id .. " ai: " ..tostring(self.curForm.aiAgent._ai)})
...
function Hero:onAStateDel(state,objectID)
    ...
    elseif state == eAState.E_GAN_DIAN then --
        self:onStateRemoveTrigger(state)
    elseif state >= eAState.E_SKILLTYPE_1_DISABLE and state <= eAState.E_SKILLTYPE_10_DISABLE then
        local keyCode = skill_type_disable[state]
        local skill   = self:getSkill(keyCode)
        if skill then
            skill:forceUpdate()
        end
    elseif state >= eAState.E_SKILLTYPE_1_DISABLE_CD and state <= eAState.E_SKILLTYPE_10_DISABLE_CD then
...
function Hero:removeBuffer(buffer)
    buffer:clear()
    table.removeItem(self.bufferList,buffer)
end
...
function Hero:getBufferIds()
    ...
    for i, targetType in ipairs(pointTargetTypes) do
        local ids = battleController.getPointBuffer(targetType,self.data.id ,self.data.attrType)
...
--第二形态的技能附带buffer
    ...
    --天使强化附带buff
    local angleStrengthenBuffs = AngelDataMgr:getAngelStregthenBuffers(self.data)
    if angleStrengthenBuffs then 
        for i, buffId in ipairs(angleStrengthenBuffs) do
            buffIds[buffId] = buffId
        end
    end
    ...
    if self.data.pet then 
        local skillID     = PetDataMgr:getPetSkill(self.data.pet.cid ,self.data.pet.star) 
        if skillID then
            local skillData   = TabDataMgr:getData("PassiveSkills",skillID)
            local buffs       = skillData.buffs[self.data.id] or skillData.buffs[0]
            if buffs then
            ...
            dump(self.data.pet)
            Box("Pet: "..tostring(self.data.pet.cid ) .." star: " ..tostring(self.data.pet.star) .." 技能不存在")
        end
    end
...
function Hero:release_()
    ...
    --移除宠物
    if self.petObjId then 
        local petObj  = petMgr:getObject(self.petObjId)
        -- Box("remove Pet Obj:" ..tostring(petObj~=nil))
        if petObj then 
            petObj:removeFromParent()
        end
        self.petObjId = nil
    end
end
...
function Hero:onDie(arg)
    ...
    local function _fadeOut( )
        self.actor:fadeOut(function()
            if BattleDataMgr:getBattleType() == EC_BattleType.ARENA  then 
                -- Box("onHeroDead:"..tostring(self:getRoleType()) .."  "..tostring(self.camp))
                self.team:remove(self)
                self:release()
                local roleType = self:getData().roleType
                if roleType == eRoleType.Hero or  
                   roleType == eRoleType.Hero_Arena then
                    battleController.onTriggerArenaRound(self)
                end
            else
                local roleType = self:getData().roleType
...
--获取精灵属性类型
function Hero:getHeroAttrType()
    return self.data.attrType or 1
end
...
function Hero:calculatePoistion()
    ...
    elseif self:getRoleType() == eRoleType.Hero_Arena  then
        local basePosition = levelParse:getBornPos(1)
        return ccp(basePosition.x + 600 ,basePosition.y) 
...
function Hero:createActor(position,hide)
    ...
    local battleType_ = BattleDataMgr:getBattleType()
    if battleType_ == EC_BattleType.ARENA then
        --TODO 竞技场战斗不直接加入到战斗场景
        --召唤怪
        if self.data.roleType == eRoleType.Monster then
            dump("召唤怪阵营:"..tostring(self.camp) .."   "..tostring(self.isAIEnable))
            self:addToBattle()
        end
        return  
    end
    ...
    elseif roleType == eRoleType.Hero_Arena then
        if self.data.job == 4 then --默认队长
            self:addToBattle()
        end
...
function Hero:_isManual()
    if battleController.getCaptain() == self then 
        return not battleController.getAutoFight()
    else
        return false
    end
end
...
function Hero:isManual(flag)
    ...
    if flag then 
        ...
        if battleController.isLockStep() then --组队下不支持自动战斗
            if self:getCampType() == eCampType.Hero then
                -- if battleController.getCaptain() == self then 
                --     return not battleController.getAutoFight()
                -- else
...
function Hero:handleMusicEvent(pramN)
    ...
    if musicDatas then
        for i, musicData in ipairs(musicDatas) do
            if not string.isNullOrEmpty(musicData.resource) then 
...
--------------------队长切换和队友助战--------------------------
--用于竞技场战斗
function Hero:addToBattle_arena()
    self:addToBattle()
    if self:getRoleType() == eRoleType.Hero then 
        battleController.setCaptain(self)
    end

    if self:getData().isPlayBornAction then
        self:doEvent(eStateEvent.BH_BORN,callback)
    else
        self:doEvent(eStateEvent.BH_STAND) 
        self:getActor():show()
        self:getActor().skeletonNode:show()
        self:setFight(true)
    end
    self:setFight(true)
end
...
function Hero:addToBattle()
    ...
    if self.data.pet then 
        if not self.petObjId then 
            local pet = GameObject.createPet(self.data.pet.cid ,self)
            self.petObjId = pet:getObjectID()
        end
    end
...
function Hero:act_useSpecifiedSkill(id,skillType)
    --Box("根据技能")
    dump({"act_useSpecifiedSkill" ,id ,skillType})
    local ret = self:castByType(skillType) 
    if not ret then 
        self:endToAI()
    end 
    return ret
...
function Hero:act_patrol(id,type_, walkDistance, walkWeight, runWeight)
    ...
    if self:_checkState(eState.ST_STAND,eState.ST_MOVEEX) then
        local pos
        if type_ == 1 or type_ == 2  then --原来需要目标
        ...
        elseif type_ == 3 then --右边走
            ...
            if BattleDataMgr:getBattleType() ~= EC_BattleType.ARENA then 
                ...
                while true do
                    maxLoopCount =  maxLoopCount - 1
                    --TODO
                    pos = ccp(myPos.x ,myPos.y)
                    local moveRect = levelParse:getMoveRect()
                    pos.x =  math.min(pos.x + _moveDis ,moveRect.origin.x + moveRect.size.width - 10)
                    pos.y =  moveRect.origin.y + moveRect.size.height *0.5
                    if levelParse:canMove(pos.x, pos.y) then
                        break
                    end
                    _moveDis =  math.min(0 ,_moveDis - 20)
                    if maxLoopCount  < 1 then
                        Box("act_patrol 死循环了3:" .. self.data.id .. " "..tostring(_moveDis))
                        pos = nil
                        break
                    end
                end
...
function Hero:getArenaFightState()
    return self.arenaFightState or 0
end
--1胜利 -1失败 
function Hero:setArenaFightState(state)
    self.arenaFightState = state
end
