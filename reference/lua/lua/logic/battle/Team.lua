...
function Team:realeaseHeros()
    ...
    self.heroList = {}  --所有未死亡的角色
    self._heroList = {} --所有我方角色包含死亡
    self._arenaHeroList = {}
end
...
function Team:createHeros(callback,host)
    ...
    for index , heroData in ipairs(self.member_) do
        ...
        elseif hero:getRoleType() == eRoleType.Hero_Arena then 
            table.insert(self._arenaHeroList, hero)
        end
        --出场动画
        if hero:isBattle() then
            if hero:getData().isPlayBornAction then
                ...
                if hero:getRoleType() == eRoleType.Hero
                or hero:getRoleType() == eRoleType.Team
                or hero:getRoleType() == eRoleType.Hero_Arena then
                    -- bornEventHero = hero
...
--获取竞技场召唤怪
function Team:getArenaMonsters()
    local monsters = {}
    for index ,hero in ipairs(self.heroList) do
        if hero:getRoleType() == eRoleType.Monster then
            table.insert(monsters,hero)
        end
    end
    return monsters
end
...
--竞技场用
function Team:getArenaHeros()
    return self._arenaHeroList
end


--竞技场用
function Team:getHero(job)
    for i,v in ipairs(self._heroList) do
        local _job = v:getData().job 
        if _job == job then 
            return v
        end
    end
...
--竞技场用
function Team:getArenaHero(job)
    for i,v in ipairs(self._arenaHeroList) do
        local _job = v:getData().job 
        if (_job -1) % 3 + 1 == job then 
            return v
        end
    end
end
