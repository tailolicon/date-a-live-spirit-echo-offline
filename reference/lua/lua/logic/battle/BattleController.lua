...
function battleController._onLoseHp(hero,hp)
    ...
    elseif campType == eCampType.Other then --统计竞技场伤害
        if hero:getRoleType() == eRoleType.Hero_Arena then
            statistics.doHurtValue(hp)
        end
    end
end
...
--设置自动战斗状态
function battleController.setAutoFight(_autoFight)
    if this.autoFight ~= _autoFight then
        this.autoFight = _autoFight
        EventMgr:dispatchEvent(eEvent.EVENT_AUTO_FIGHT_STATE_CHANGE,_autoFight)
    end
end

function battleController.getAutoFight()
...
function battleController.setCaptain(hero)
    ...
    -- Box("setCaptain"..tostring(hero == nil) ..debug.traceback())
...
function battleController.herosEnter()
    ...
    -- Box("battleController.herosEnter-----")
    ...
    this.team:createHeros(function()
        ...
        if this.nState > eBState.BS_NOT_INIT then
            ...
            this.showCounDown(function()
                ...
                if BattleDataMgr:getBattleType() == EC_BattleType.ARENA then 
                    -- Box("开始第一波战斗")
                    --竞技场战斗不自动出场，通过手动调用开始战斗
                    this.startArenaRound(1)
                end
...
--开始竞技场战斗机 回合 一共三回合
function battleController.startArenaRound(round)
    this.arenaRound  = round
    EventMgr:dispatchEvent(eEvent.EVENT_PREWARTIP_SHOW)
    
    local timerHandle = TFDirector:addTimer(2000, 1, nil, function ()
        TFDirector:removeTimer(timerHandle)
        local hero  = this.team:getHero(this.arenaRound)
        local enemy = this.team:getArenaHero(this.arenaRound)
        hero:addToBattle_arena()
        enemy:addToBattle_arena()
    end)
end


function battleController.getArenaRound()
    return this.arenaRound or 0
end

--竞技场战斗时间结束
function battleController.onArenaRoundTimeout()

   local hero   = this.team:getHero(this.arenaRound)
    local enemy = this.team:getArenaHero(this.arenaRound)
    if hero:getHp() > enemy:getHp() then 
        this.onTriggerArenaRound(enemy)
    else
        this.onTriggerArenaRound(hero)  
    end 
...
function battleController.onTriggerArenaRound(hero)
    this.roundState = this.roundState or {}
    local data    = hero:getData()
    local job     = data.job
    local round   = (job - 1)%3 + 1
    if this.roundState[round] then
        return 
    end
    this.roundState[round] =  true
    local timerHandle = TFDirector:addTimer(2000, 1, nil, function ()
        this._onTriggerArenaRound(hero)
    end)

end

function battleController._onTriggerArenaRound(hero)
    ...
    local job     = data.job
    local round   = (job - 1)%3 + 1
    if this.arenaRound ~= round then 
        return 
    end

    local success = MainPlayer:getPlayerId() ~= hero:getPID() 
    this.data.roundData[this.arenaRound].success = success
    --清理回合战斗角色
    local hero  = this.team:getHero(this.arenaRound)
    local enemy = this.team:getArenaHero(this.arenaRound)
    hero:setArenaFightState(success and 1 or -1)
    enemy:setArenaFightState(success and -1 or 1)
    this.team:remove(hero)
    this.team:remove(enemy)
    hero:removeAllEffect()
    enemy:removeAllEffect()
    hero:removeAllBuff()
    enemy:removeAllBuff()
    hero:release()
    enemy:release()

    local monsters = this.team:getArenaMonsters()
    -- if #monsters > 0 then 
    --     Box("移除Monster".. #monsters)
    -- end
    for i, monster in ipairs(monsters) do
        this.team:remove(monster)
        monster:removeAllEffect()
        monster:removeAllBuff()
        monster:release()
    end

    --移除所有特效
    local effects = BattleMgr.getEffectMgr()
    for i,effect in pairs(effects:getObjects()) do
        effect:preRemove()
    end


    -- print("REMOVE: "..hero:getName())
    -- print("REMOVE: "..enemy:getName())

    if this.arenaRound < 3 then 
        -- Box("回合结束:"..  this.arenaRound)
        this.arenaRound = this.arenaRound + 1
        this.startArenaRound(this.arenaRound)
        local time = BattleDataMgr:getLevelCfg().time  
        victoryDecide.setRemainime(time)
    else
    
        -- Box("战斗结束")
        local winCount = 0
        for i,v in ipairs(this.data.roundData) do
            if v.success then 
               winCount = winCount + 1 
            end
        end
        dump(this.data.roundData)
    -- Box("所有战斗结束")
        local bWin = winCount > 1
...
function battleController.requestFightingOver()
    ...
    elseif battleType == EC_BattleType.ARENA then
        local levelId = BattleDataMgr:getPointId()
        local reachStar = statistics.calReachStar()
        local pickUpTypeCount = statistics.pickUpTypeCount
        local pickUpCount     = statistics.pickUpCount
        ...
        --发送竞技场结算数据
        ArenaDataMgr:send_DUNGEON_REQ_ARENA_OVER(this.data.roundData)
...
function battleController.eventCheck(hero)
    ...
    if this.isLockStep() then
        ...
        elseif this.levelCfg_.dungeonType == EC_FBLevelType.ARENA then   --角色死亡检查
            -- this.onTriggerArenaRound(hero)
        else
...
--获取关卡加成buffer
function battleController.getPointBuffer(targetType,heroId,heroType)
    local bufferIds = {}
    local data = BattleDataMgr:getLevelCfg()
    ...
    --竞技场buff    
    if data.dungeonType == EC_FBLevelType.ARENA then
          -- Box("11111")
        local _buffIds = ArenaDataMgr:getRegionBuffs()
        if _buffIds and #_buffIds > 0 then
            table.insertTo(bufferIds, _buffIds)
        end
        -- dump(_buffIds)
        -- dump(bufferIds)
        -- Box("11111")
    ...
    --元素试炼buff
    if data.dungeonType == EC_FBLevelType.ELEMENT_TRIAL  then
        if targetType == enum.ePointTargetType.Hero then 
            local heroQuality = 0
            local heroData = TabDataMgr:getData("Hero" ,heroId)
            if heroData then 
                heroQuality = heroData.rarity      
            end
            local elementTrainDungeonLevelCfg   = TabDataMgr:getData("ElementTrainDungeonLevel" ,data.id)
            -- Box("heroType:"..tostring(heroType))
            for i,_heroType in ipairs(elementTrainDungeonLevelCfg.heroType or {}) do
                -- dump({_heroType, " -- ",heroType})
                if _heroType == heroType then 
                    for i, _bufferId in ipairs(elementTrainDungeonLevelCfg.buffId or {}) do
                        table.insert(bufferIds, _bufferId)
                    end


                    for k, v in ipairs(elementTrainDungeonLevelCfg.heroQualityBuff or {}) do
                        -- dump({heroQuality , " -- ",v })
                        if v[2] == heroQuality then 
                            table.insert(bufferIds, v[1])
                        end
                    end
            ...
            if elementTrainDungeonLevelCfg.levelDebuffId then 
                for i, _bufferId in ipairs(elementTrainDungeonLevelCfg.levelDebuffId) do
                    table.insert(bufferIds, _bufferId)
                end
            end
            -- dump(elementTrainDungeonLevelCfg)
            -- dump(bufferIds)
            -- Box("----------------------heroQuality :".. tostring(heroQuality) .." heroType ".. tostring(heroType) .." heroId：" ..tostring(heroId))
