...
--宠物管理器
local petMgr      = BattleMgr.getPetMgr()
--资源类型
local eResType     =
{
    IMAGE    = 1 , --图片类型
...
function GameObject:createRenderNode()
    ...
    elseif self:getResType() == eResType.SKELETON2 then
        self.renderNode = SkeletonAnimation:create(self:getRes())
        self.renderNode:setScale(self:getScale_())
        self.renderNode:setPosition(ccp(0,0))
        self:addChild(self.renderNode,1)
    end
    ...
    -- Box("createRenderNode")
end


function GameObject:playAnimation(name, loop, completeCallback)
    if self:getResType() == eResType.SKELETON or self:getResType() == eResType.SKELETON2 then
...
local Pet = class("Pet",GameObject)

function Pet:ctor(data,host)
    self.super.ctor(self,data)

    self.host  = host
    self.speed = 5
    self.followingDistance = 80  -- 跟随距离
    self.updateInterval = 0.4   -- 更新间隔（秒）
    self.lastUpdate = 0


    self.shadow = TFImage:create("ui/battle/shadow.png")
    self.shadow:setScale(1) 
    -- self.shadow:setOpacity(1)
    self.shadow:setPosition(ccp(0,0))
    self:addChild(self.shadow,1)


    self:createRenderNode()

    self.renderNode:setScale(data.scale)
    self.renderNode:setPosition(ccp(0,0))

    self:playStand()
    self:setPosition(self.host:getPosition())
    self:active()

      -- self.velocity = ccp(0,0)
      -- self.offset = ccp(50,0)
      -- self.smoothing = 0.1
end
...
function Pet:playStand()
    if self.animation ~= self.data.idleAction  then  
        self.animation = self.data.idleAction 
        self:playAnimation(self.data.idleAction,true)
    end
end

function Pet:playMove()
    if self.animation ~= "move"  then  
        self.animation = "move" 
...
function Pet:_onExit()
    print("--------------------------------------Pet",self.id ,"_onExit")
    petMgr:remove(self)
    -- Box("Pet:_onExit")
end

function Pet:_onEnter()
    print("Pet",self.id ,"_onEnter")
    petMgr:add(self)

end

function Pet:canHurt()
    return false
end


function Pet:getSortY()
    --return 640
    return self:getPositionY()
end
...
function Pet:update_(dt)
    ...
    if self.lastUpdate  > self.updateInterval then
        ...
        -- 计算与宿主的距离
        local dx = self.host:getPositionX() - _x
        local dy = self.host:getPositionY() - _y
        local distance = math.sqrt(dx * dx + dy * dy)
        -- 如果超过跟随距离，则移动
        local threshold = 0.5
        if distance > self.followingDistance + 10  then
            -- 归一化方向向量
            local nx, ny = dx / distance, dy / distance
            local moveDist = math.min(self.speed * dt * 60, distance - self.followingDistance)
            -- 向宿主移动
            local x =  _x + nx * moveDist
            local y =  _y + ny * moveDist
            ...
            self.lastUpdate = self.lastUpdate - self.updateInterval
            self:playStand()
        end
        -- self:setPosition(ccp(self.host:getPositionX() , self.host:getPositionY() ))
        -- dump({self:getPosition() ,self.host:getPosition() ,_x , _y , dt})
...
function Pet:update(dt)
    ...
    if self.lastUpdate  > self.updateInterval then
        ...
        local dx = self.host:getPositionX() - _x
        local dy = self.host:getPositionY() - _y
        local ads_dx = math.abs(dx)
        local ads_dy = math.abs(dy)
        if ads_dx > 100 or ads_dy > 40 then 
            local moveX = math.min(self.speed * dt * 60      , ads_dx) *_sign(dx)
            local moveY = math.min(self.speed * dt * 60 * 0.5, ads_dy) *_sign(dy) 
            local x =  _x + moveX
            local y =  _y + moveY
            -- dump({"move" ,x,y})
            self:setPosition(x , y)
...
function GameObject.createPet(id ,host)
    local cfg = PetDataMgr:getPetCfg(id) 
    local data =
    {
        resType  = 3,
        scale    = cfg.petSize,
        flipX    = false,
        resource = cfg.paint,
        idleAction = cfg.defaultAct or "idle"
    }
    return Pet:new(data ,host)
end
