...
local function _checkScriptTest()
    --约会测试模式判定
    if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32 then
        local currentScene = Public:currentScene()
        if currentScene and currentScene.__cname == "DatingEditScene" then
            isScriptTest = true  
        end
    end
end
...
function DatingScriptView:getAutoSpeed()
    local id    = MainPlayer:getPlayerId() or ""
    local speed = UserDefalt:getIntegerForKey("DatingAutoSpeed"..id)
    return speed
end

function DatingScriptView:setAutoSpeed(speed)
    local id = MainPlayer:getPlayerId() or ""
    UserDefalt:setIntegerForKey("DatingAutoSpeed"..id, speed)
    UserDefalt:flush()
end

function DatingScriptView:getIsSkip()
    local id    = MainPlayer:getPlayerId() or ""
    local speed = UserDefalt:getBoolForKey("DatingSkip"..id)
    return speed
end

function DatingScriptView:setIsSkip(skip)
    local id    = MainPlayer:getPlayerId() or ""
    UserDefalt:setBoolForKey("DatingSkip"..id ,skip)
    UserDefalt:flush()
end
...
function DatingScriptView:ctor(isFubenShowBackBtn,isNoF)
    ...
    self.isSkip    = self:getIsSkip()
    self.autoSpeed = self:getAutoSpeed()
    self.isAuto    = self.autoSpeed > 0
...
function DatingScriptView:initUI(ui)
    ...
    --如果跳过按钮是隐藏的屏蔽跳过功能
    if self.skipButton.isHide then 
        self.isSkip = false
    end
...
function DatingScriptView:initButtonList()
    ...
    self.autoButton.select = self.autoButton:getChildByName("Image_select")
      self.autoButton.Image_autoSpeed = {}
    for i=1,3 do
        self.autoButton.Image_autoSpeed[i] = TFDirector:getChildByPath(self.autoButton.select,"Image_autoSpeed"..i)
    end
...
function DatingScriptView:playVoice(time)
    ...
    self.Panel_pro:timeOut(function()
        ...
        if self.voiceHandle and self.voiceHandle ~= -1 then
            local baseVol = SettingDataMgr:getSoundMainVal()
...
function DatingScriptView:playSoundEffect(pathName)
    ...
    if self.soundEffectHandle and self.soundEffectHandle ~= -1 then
...
function DatingScriptView:refreshAutoSpeed()
    for i=1,3 do
        self.autoButton.Image_autoSpeed[i]:setVisible(self.autoSpeed == i)
    end
end
--注册事件
...
function DatingScriptView:onClose()
    print("----------------------------------保存约会状态    autoSpeed:"..tostring(self.autoSpeed) .. "  skip:"..tostring(self.isSkip))
    self:setAutoSpeed(self.autoSpeed)
    self:setIsSkip(self.isSkip)
    --关闭界面时置空回调
