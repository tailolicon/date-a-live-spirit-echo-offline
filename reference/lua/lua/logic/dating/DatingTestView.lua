...
function DatingTestView:initUI(ui)
    ...
    self.Button_cgEditLast = TFDirector:getChildByPath(self.ui, "Button_cgEditLast")
    self.Button_cgEditNew = TFDirector:getChildByPath(self.ui, "Button_cgEditNew")
...
function DatingTestView:updateSelectListView(selectType,findList)
    self.selectListView:removeAllItems()
    self.selectItemList = {}
    if selectType == self.selectTypeList.ScriptId then
        if self.curDatingTableName ~= "" and self.datingTableConfigList[self.curDatingTableName] then
            local scriptIdList = findList or self.datingTableConfigList[self.curDatingTableName].scriptIdList
    ...
    elseif selectType == self.selectTypeList.DatingName then
        local list = findList or self.datingTableConfigList
...
function DatingTestView:onTouchSendBtn()
    ...
    if self.TextField_input2:getText() ~= "" then
        ...
        if not self.datingTableConfigList[self.curDatingTableName] then
            Utils:showTips(100000031)
            return
        elseif self.TextField_input1:getText() == "" then
            Utils:showTips("请输入正确的剧本名！")
            return
        elseif self.TextField_input2:getText() == "" then
            Utils:showTips("请输入正确的剧情ID！")
            return
        end
...
function DatingTestView:registerEvents()
    ...
    local TextButton_find = TFDirector:getChildByPath(self.ui, "TextButton_find")
    local TextField_findName = TFDirector:getChildByPath(self.ui, "TextField_findName")
    local Button_nback = TFDirector:getChildByPath(self.ui, "Button_nback"):show()

    TextButton_find:onClick(function()
        self.cgFindFileNamelList = {}
        self.isFind = false
        self.findList = {}
        if self.curSelectType == self.selectTypeList.ScriptId then
            local scriptIdList = self.datingTableConfigList[self.curDatingTableName].scriptIdList
            for i,v in ipairs(scriptIdList) do
                if string.find(v,TextField_findName:getText()) then
                    self.isFind = true
                    table.insert(self.findList,v)
                end
            end
        elseif self.curSelectType == self.selectTypeList.DatingName then
            for k,v in pairs(self.datingTableConfigList) do
                if string.find(v.name,TextField_findName:getText()) then
                    self.isFind = true
                    table.insert(self.findList,v)
                end
            end
        ...
        if self.isFind then
            --Button_nback:show()
            TextButton_find:hide()
            TextField_findName:hide()
            self:showSelectListView(self.curSelectType,self.findList)
        else
            TextField_findName:setText("")
            Utils:showTips("没有找到任何剧本，请输入正确的剧本！")
        end
    end)

    Button_nback:onClick(function()
        if self.isFind then
            self.isFind = false
            --Button_nback:hide()
            TextButton_find:show()
            TextField_findName:show()
            TextField_findName:setText("")
            self:showSelectListView(self.curSelectType)
        else
            self:hideSelectListView()
        end
    end)
    ...
    self.Button_cgEditLast:onClick(function()
            local specialEffectsTestLayer = require("lua.logic.dating.SpecialEffectsTestLayer"):new(true)
            AlertManager:addLayer(specialEffectsTestLayer,AlertManager.BLOCK,AlertManager.TWEEN_NONE)
            AlertManager:show()
        end)
    self.Button_cgEditNew:onClick(function()
        local specialEffectsTestLayer = require("lua.logic.dating.SpecialEffectsTestLayer"):new(false)
        AlertManager:addLayer(specialEffectsTestLayer,AlertManager.BLOCK,AlertManager.TWEEN_NONE)
        AlertManager:show()
    end)
end
