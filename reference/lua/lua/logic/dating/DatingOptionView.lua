...
function DatingOptionView:initOption1()
    ...
    for idx=1,3 do
        ...
        self["button_option_" .. i].Label_timeout = self["button_option_" .. i]:getChild("Label_timeout")
        self["button_option_" .. i].Label_timeout:hide()
...
function DatingOptionView:removeEvents()
    self:stopTimer()
end

function DatingOptionView:checkAutoSelect1()
    if SettingDataMgr:getOptionSkipVal() == 1 then 
        for i=1,3 do
            local option =  self["button_option_" .. i]
            if option:isVisible() and  option.isenable then 
                self:startTimer(option)
                break
            end
        end
    end
...
function DatingOptionView:onAutoSelectOption1()
    local _index = 0
    for index=1,3 do
        local button_option = self["button_option_" .. index]
        button_option:setTouchEnabled(false)
        if button_option == self.buttonOption then 
            _index = index
        end

    end
    self:onTouchButtonOption(_index)
end

function DatingOptionView:startTimer(buttonOption)
    self:stopTimer()
    self.buttonOption  = buttonOption
    self.buttonOption.Label_timeout:show()
    self.startTime = 5
    self.buttonOption.Label_timeout:setTextById(18000329,self.startTime)   -- 替换语言表
    self.nTimerId  = TFTimerManager:addTimer(1000, 5, 
    function() --时间刷新
        self:onAutoSelectOption1()
        self:stopTimer()
    end, 
    function() --回调
...
function DatingOptionView:stopTimer()
    if self.buttonOption then  
        self.buttonOption.Label_timeout:hide()
    end
    if self.nTimerId ~= nil then 
        TFTimerManager:removeTimer(self.nTimerId)
        self.nTimerId = nil
    end
end
