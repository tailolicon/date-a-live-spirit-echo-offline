...
function PlayerSetting:initUI(ui)
    ...
    local _Label_name = TFDirector:getChildByPath(self.Panel_spr_item,"Label_name")
    Utils:setFontName(_Label_name)
    self.Panel_medal_item = TFDirector:getChildByPath(self.Panel_prefab, "Panel_medal_item")
    self.Panel_phoneBind = TFDirector:getChildByPath(self.ui,"Panel_phoneBind"):hide()
...
function PlayerSetting:refreshLeft()
    ...
    Utils:createHeroModel_(self.Image_fairy ,skinid, "battleSize")
...
function PlayerSetting:initInfo()
    ...
    self.Button_account_info_logout = TFDirector:getChildByPath(self.Panel_player_info,"Button_account_info_logout") :hide()
    ...
    self.Button_lianxi = TFDirector:getChildByPath(self.Panel_player_info, "Button_lianxi"):hide()
    self.Button_mima = TFDirector:getChildByPath(self.Panel_player_info, "Button_mima"):hide()
...
function PlayerSetting:registerEvents()
    ...
    --if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID and HeitaoSdk  then
        self.Button_mima:show()
        local label_mima = TFDirector:getChildByPath(self.Button_mima, "Label_mima")
        label_mima:setTextById(1800100)
        --修改密码 修改用户中心
        self.Button_mima:onClick(function()
            if HeitaoSdk then
                HeitaoSdk.userCenter()
            else
                -- Box("用户中心")
                local logoutView = require("lua.logic.playerInfo.LogoutView"):new()
                AlertManager:addLayer(logoutView)
